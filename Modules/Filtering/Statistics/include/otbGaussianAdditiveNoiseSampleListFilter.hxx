...
GaussianAdditiveNoiseSampleListFilter<TInputSampleList, TOutputSampleList>::GaussianAdditiveNoiseSampleListFilter()
{
  m_Mean              = 0.;
  m_Variance          = 1e-3;
...
template <class TInputSampleList, class TOutputSampleList>
void GaussianAdditiveNoiseSampleListFilter<TInputSampleList, TOutputSampleList>::GenerateRandomSequence()
{
...
template <class TInputSampleList, class TOutputSampleList>
void GaussianAdditiveNoiseSampleListFilter<TInputSampleList, TOutputSampleList>::GenerateData()
{
  ...
  for (unsigned int currIteration = 0; currIteration < m_NumberOfIteration; currIteration++)
  {
    ...
    while (inputIt != inputSampleListPtr->End())
    {
      ...
      {
        currentOutputMeasurement[idx] =
            static_cast<OutputValueType>((static_cast<double>(currentInputMeasurement[idx]) + m_WhiteGaussianNoiseCoefficients[idx]));
      }
...
template <class TInputSampleList, class TOutputSampleList>
void GaussianAdditiveNoiseSampleListFilter<TInputSampleList, TOutputSampleList>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
