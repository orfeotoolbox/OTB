...
template <class TInputPixel1, class TInputPixel2, class TOutputPixel>
class ITK_EXPORT AlphaBlendingFunctor : public AlphaBlendingFunctorBase<TInputPixel1, TInputPixel2, TOutputPixel>
{
public:
  ...
  inline OutputPixelType operator()(InputPixel1Type input1, InputPixel2Type input2) const
  {
    ...
    resp = static_cast<OutputPixelType>(std::floor(0.5 + (1.0 - alpha) * static_cast<double>(input1) + alpha * static_cast<double>(input2)));
    return resp;
  }
...
class ITK_EXPORT AlphaBlendingFunctor<itk::RGBAPixel<TInputInternalPixel1>, itk::RGBAPixel<TInputInternalPixel2>, itk::RGBAPixel<TOutputInternalPixel>>
...
public:
  ...
  inline OutputPixelType operator()(InputPixel1Type input1, InputPixel2Type input2) const
  {
    ...
    resp.SetRed(static_cast<InternalOutputPixelType>(
        std::floor(0.5 + (1.0 - alpha) * static_cast<double>(input1.GetRed()) + alpha * static_cast<double>(input2.GetRed()))));
    resp.SetGreen(static_cast<InternalOutputPixelType>(
        std::floor(0.5 + (1.0 - alpha) * static_cast<double>(input1.GetGreen()) + alpha * static_cast<double>(input2.GetGreen()))));
    resp.SetBlue(static_cast<InternalOutputPixelType>(
        std::floor(0.5 + (1.0 - alpha) * static_cast<double>(input1.GetBlue()) + alpha * static_cast<double>(input2.GetBlue()))));
    return resp;
  }
