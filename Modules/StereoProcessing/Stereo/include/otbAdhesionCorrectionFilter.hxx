...
AdhesionCorrectionFilter<TImage, TMask>::AdhesionCorrectionFilter()
{
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::SetMedianDisparityInput(const TImage* medianmap)
{
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::SetMedianMaskInput(const TMask* medianmask)
{
  // Process object is not const-correct so the const casting is required.
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::SetEdgesDisparityInput(const TImage* cannymedianmap)
{
  // Process object is not const-correct so the const casting is required.
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::SetSubPixelMaskInput(const TMask* subpixelmask)
{
  // Process object is not const-correct so the const casting is required.
...
template <class TImage, class TMask>
const TImage* AdhesionCorrectionFilter<TImage, TMask>::GetMedianDisparityInput()
{
...
template <class TImage, class TMask>
const TMask* AdhesionCorrectionFilter<TImage, TMask>::GetMedianMaskInput()
{
...
template <class TImage, class TMask>
const TImage* AdhesionCorrectionFilter<TImage, TMask>::GetEdgesDisparityInput()
{
...
template <class TImage, class TMask>
const TMask* AdhesionCorrectionFilter<TImage, TMask>::GetSubPixelMaskInput()
{
...
template <class TImage, class TMask>
TMask* AdhesionCorrectionFilter<TImage, TMask>::GetOutputMask()
{
...
template <class TImage, class TMask>
TImage* AdhesionCorrectionFilter<TImage, TMask>::GetOutputRiskEdges()
{
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::GenerateOutputInformation()
{
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::GenerateInputRequestedRegion()
{
...
template <class TImage, class TMask>
void AdhesionCorrectionFilter<TImage, TMask>::GenerateData()
{
  // Allocate outputs
  ...
  while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1])
  {
        ...
        new_disparityIt.GetIndex()[1] < static_cast<int>(m_ImageSize[1]) - win)
    {
      ...
      int k = 0;
      while (new_disparityIt.GetIndex()[0] <
             static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - win)
      {
        index_pos = new_disparityIt.GetIndex();
        ...
        else // Disparity map discontinuities
        {
          if (canny_disparity->GetPixel(index_pos) > m_DiscontinuityThreshold)
          {
            ...
            for (k = 0; k < 8; k++)
            {
              ...
              index[1] = index_pos[1];
              if (old_maskPtr->GetPixel(index) == 0)
                pp++;
            }
            if (pp == 0)
            {
          ...
          if ((disparity_jumpIt.Get() == 1 || disparity_jumpIt.Get() == 2) && disparity_jumpIt.Get() != disparity_jump->GetPixel(index_pos))
          {
            for (int i = -2 * win; i <= 2 * win; i++)
            {
              ...
              if (index2[0] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] &&
                  index2[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) +
                                  outputPtr->GetRequestedRegion().GetIndex()[1]) // add this condition to avoid seg fault
              {
  ...
  while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1])
  {
    ...
    while (new_disparityIt.GetIndex()[0] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1)
    {
      ...
      {
        int l = -1;
        while (index_pos[0] + l >= outputPtr->GetRequestedRegion().GetIndex()[0] && l >= -big_dist &&
               index_pos[0] + l < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] &&
               index_pos[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1])
        {
          index    = index_pos;
  ...
  while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1])
  {
    ...
    while (new_disparityIt.GetIndex()[0] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1)
    {
      ...
      if (risk_edgesIt.Get() == 1)
      {
        ...
        risk_edgesIt.SetIndex(index_pos);
        if (pp == 0)
          risk_edgesIt.Set(0); // remove isolate points
        if (pp == 1)
          risk_edgesIt.Set(2);
        if (pp == 2)
        {
      ...
      if (risk_edgesIt.Get() == 2)
      {
        ...
        while (new_step == 1)
        {
          ...
          { // Ignore merging cases
            ...
            if (canny_edgesIt.Get() > m_DiscontinuityHighThreshold && std::abs(canny_edgesIt.Get() - canny_edges->GetPixel(index_pos_old)) < m_MaxEdgeGap)
            {
          ...
          for (int j = -big_win; j <= big_win; j++)
          {
            for (int i = -big_win; i <= big_win; i++)
            {
              if (index_pos[0] + i >= old_disparityPtr->GetRequestedRegion().GetIndex()[0] &&
                  index_pos[0] + i <
                      static_cast<int>(old_disparityPtr->GetRequestedRegion().GetSize()[0]) + old_disparityPtr->GetRequestedRegion().GetIndex()[0] &&
                  index_pos[1] + j >= old_disparityPtr->GetRequestedRegion().GetIndex()[1] &&
                  index_pos[1] + j <
                      static_cast<int>(old_disparityPtr->GetRequestedRegion().GetSize()[1]) + old_disparityPtr->GetRequestedRegion().GetIndex()[1])
              {
                ...
                if (old_maskIt.Get() == 1)
                {
                  if (old_disparityPtr->GetPixel(index_pos0) > m_max)
                    m_max = old_disparityPtr->GetPixel(index_pos0);
                  if (old_disparityPtr->GetPixel(index_pos0) < m_min)
                    m_min = old_disparityPtr->GetPixel(index_pos0);
                  Count++;
                }
                new_disparityIt.SetIndex(index_pos);
              }
  ...
  while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1] - 1)
  {
    ...
    while (new_disparityIt.GetIndex()[0] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1)
    {
      ...
      { // left
        ...
        if (disparity_jump->GetPixel(index) == 3)
        {
          ...
          { // 2a adhesion
            for (int j = 1; j <= win; j++)
            {
              index2[0] = index_pos[0] + i + j;
              index2[1] = index_pos[1];
              if (outputriskedgesPtr->GetPixel(index2) != 0)
                pp2++;
            }
            index[0] = index_pos[0] + i;
            disparity_jumpIt.SetIndex(index);
        ...
        index[0] = index_pos[0] + l;
        index[1] = index_pos[1];
        while ((new_disparityIt.GetIndex()[0] + l) <
                   static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1 &&
               l >= 0 && disparity_jump->GetPixel(index) != 2)
        {
          l--;
          index[0] = index_pos[0] + l;
        }
        ...
        while ((new_disparityIt.GetIndex()[0] + l) <
                   static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1 &&
               l >= 0 && disparity_jump->GetPixel(index) != 4)
        {
          l--;
          index[0] = index_pos[0] + l;
        }
        ...
        for (int i = -win + 1; i <= win - 1; i++)
        {
          for (int k = -win + 1; k <= win - 1; k++)
          {
            ...
            if (index2[0] >= outputPtr->GetRequestedRegion().GetIndex()[0] && index2[1] >= outputPtr->GetRequestedRegion().GetIndex()[1] &&
    ...
    while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1] - 1)
    {
      ...
      else
      {
        ...
        { // only small holes
          ...
          if (old_disparityPtr->GetPixel(index_pos) > disp)
            disparity_jump2It.Set(5);
          index[0] = index_pos[0] - 1;
          index[1] = index_pos[1];
          disparity_jump2It.SetIndex(index);
          if (old_disparityPtr->GetPixel(index_pos) < disp)
            disparity_jump2It.Set(6);
        }
        k    = 0;
      ...
      if (disparity_jump2->GetPixel(index_pos) == 7)
      {
        ...
        for (int i = 0; i <= big_dist; i++)
        {
          if (index_pos[1] + i < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1])
          {
            ...
            if (old_maskPtr->GetPixel(index_pos0) == 0)
              k++;
            else
            {
              ...
              { // only small holes
                ...
                if (old_disparityPtr->GetPixel(index_pos0) > disp)
                  disparity_jump2It.Set(8);
                index[0] = index_pos0[0] - 1;
                index[1] = index_pos0[1];
                disparity_jump2It.SetIndex(index);
                if (old_disparityPtr->GetPixel(index_pos0) < disp)
                  disparity_jump2It.Set(9);
              }
              k    = 0;
  ...
  while (new_disparityIt.GetIndex()[0] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[0]) + outputPtr->GetRequestedRegion().GetIndex()[0] - 1)
  {
    ...
    while (new_disparityIt.GetIndex()[1] < static_cast<int>(outputPtr->GetRequestedRegion().GetSize()[1]) + outputPtr->GetRequestedRegion().GetIndex()[1] - 1)
    {
      ...
      if (outputriskedgesPtr->GetPixel(index_pos) != 0)
      {
        ...
        index[1] = index_pos[1] + l;
        disparity_jump2It.SetIndex(index);
        if (disparity_jump2It.Get() == 5 && l != 0)
          disparity_jump2It.Set(0);
        for (int i = l; i <= 0; i++)
        {
        ...
        index[1] = index_pos[1] + l;
        disparity_jump2It.SetIndex(index);
        if (disparity_jump2It.Get() == 6 && l != 0)
          disparity_jump2It.Set(0);
        for (int i = l; i >= 0; i--)
        {
          index[1] = index_pos[1] + i;
        ...
        index[1] = index_pos[1] + l;
        disparity_jump2It.SetIndex(index);
        if (disparity_jump2It.Get() == 8 && l != 0)
          disparity_jump2It.Set(0);
        for (int i = l; i <= 0; i++)
        {
          index[1] = index_pos[1] + i;
        ...
        index[1] = index_pos[1] + l;
        disparity_jump2It.SetIndex(index);
        if (disparity_jump2It.Get() == 9 && l != 0)
          disparity_jump2It.Set(0);
        for (int i = l; i >= 0; i--)
        {
          index[1] = index_pos[1] + i;
      ...
      if (new_maskIt.Get() != 0 && subpixelmaskPtr->GetPixel(index_pos) != 0)
      {
        ...
        for (int j = -win + 1; j <= win - 1; j++)
        {
          for (int i = -win + 1; i <= win - 1; i++)
          {
            ...
            index_pos0[1] = index_pos[1] + j;
            new_maskIt.SetIndex(index_pos0);
            if (new_maskIt.Get() != 0 && subpixelmaskPtr->GetPixel(index_pos0) != 0)
              nb_disp++;
          }
        }
      }
