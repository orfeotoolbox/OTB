...
OGRDataResampler::findBestSize(unsigned int tot)
{
   ...
   for(unsigned int size=m_MaxSamplingTabSize; size>=20; size--)
   if (tot%size == 0)
      return size;
      
   otbWarningMacro(<<"In OGRDataResampler::findBestSize, prime number > m_MaxSamplingTabSize." << std::endl );  
   return m_MinSamplingTabSize;
}
...
if (!m_alreadyPrepared)
{
   ...
   for(;itRates !=m_RatesbyClass.end(); ++itRates)
   {
      ...
      unsigned int T2 = findBestSize(itRates->second.tot/T1*(T1-N1));
...
OGRDataResampler::TakeSample(std::string className)
{
   ...
   //if (className.compare(std::string("3")) == 0)
