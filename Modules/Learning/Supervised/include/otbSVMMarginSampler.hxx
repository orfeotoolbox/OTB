...
SVMMarginSampler<TSample, TModel>::SVMMarginSampler()
{
  m_NumberOfCandidates = 10;
}

template <class TSample, class TModel>
void SVMMarginSampler<TSample, TModel>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
  Superclass::PrintSelf(os, indent);
}

template <class TSample, class TModel>
void SVMMarginSampler<TSample, TModel>::GenerateData()
{
  if (!m_Model)
  {
...
template <class TSample, class TModel>
void SVMMarginSampler<TSample, TModel>::DoMarginSampling()
{
  ...
  {
    otbMsgDevMacro("Sample " << idDistVector[i].first << " (distance= " << idDistVector[i].second << ")") m_MarginSamples.push_back(idDistVector[i].first);
  }
}
