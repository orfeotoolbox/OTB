...
inline GDALDataset& otb::ogr::DataSource::ogr()
{
  assert(m_DataSource && "GDALDataset not initialized");
  return *m_DataSource;
}

inline otb::ogr::Layer otb::ogr::DataSource::GetLayer(size_t i)
{
...
inline otb::ogr::Layer const otb::ogr::DataSource::GetLayer(size_t i) const
{
  return const_cast<DataSource*>(this)->GetLayer(i);
}

inline otb::ogr::Layer const otb::ogr::DataSource::GetLayer(std::string const& name) const
{
  return const_cast<DataSource*>(this)->GetLayer(name);
}


inline otb::ogr::Layer const otb::ogr::DataSource::GetLayerChecked(size_t i) const
{
  return const_cast<DataSource*>(this)->GetLayerChecked(i);
}

inline OGRLayer* otb::ogr::DataSource::GetLayerUnchecked(size_t i) const
{
...
otb::ogr::DataSource::layer_iter<Value>::layer_iter(container_type& datasource, size_t index) : m_DataSource(&datasource), m_index(index)
{
}

template <class Value>
otb::ogr::DataSource::layer_iter<Value>::layer_iter() : m_DataSource(0), m_index(0)
{
}

template <class Value>
template <class OtherValue>
otb::ogr::DataSource::layer_iter<Value>::layer_iter(otb::ogr::DataSource::layer_iter<OtherValue> const& other,
...
{
  assert(m_DataSource && int(m_index) < m_DataSource->GetLayersCount() && "cannot increment past end()");
  ++m_index;
}
...
{
  assert(m_DataSource && int(m_index) < m_DataSource->GetLayersCount() && "cannot dereference past end()");
