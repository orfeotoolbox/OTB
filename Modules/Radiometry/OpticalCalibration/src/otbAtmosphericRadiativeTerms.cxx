...
/**PrintSelf method */
void AtmosphericRadiativeTermsSingleChannel::PrintSelf(std::ostream& os, itk::Indent indent) const
{
...
/**CONSTRUCTOR. */
AtmosphericRadiativeTerms::AtmosphericRadiativeTerms()
{
  m_Values.clear();
  m_IsInitialized = false;
}

void AtmosphericRadiativeTerms::ValuesInitialization(DataVectorSizeType nbChannel)
{
...
void AtmosphericRadiativeTerms::SetIntrinsicAtmosphericReflectances(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
...
void AtmosphericRadiativeTerms::SetSphericalAlbedos(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetTotalGaseousTransmissions(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetDownwardTransmittances(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetUpwardTransmittances(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittances(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetUpwardDirectTransmittances(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittancesForRayleigh(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittancesForAerosol(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetWavelengthSpectralBand(const DataVectorType& vect)
{
  if (!m_IsInitialized)
  {
    this->ValuesInitialization(vect.size());
...
void AtmosphericRadiativeTerms::SetValueByIndex(unsigned int id, const ValueType& val)
{
  if (m_IsInitialized)
  {
...
void AtmosphericRadiativeTerms::SetIntrinsicAtmosphericReflectance(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetSphericalAlbedo(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetTotalGaseousTransmission(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetDownwardTransmittance(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetUpwardTransmittance(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittance(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetUpwardDirectTransmittance(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittanceForRayleigh(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetUpwardDiffuseTransmittanceForAerosol(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
void AtmosphericRadiativeTerms::SetWavelengthSpectralBand(unsigned int id, const double& val)
{
  if (m_Values.size() < id + 1)
  {
    while (id >= m_Values.size())
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetIntrinsicAtmosphericReflectances() const
{
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetSphericalAlbedos() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetTotalGaseousTransmissions() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetDownwardTransmittances() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetUpwardTransmittances() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittances() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetUpwardDirectTransmittances() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittancesForRayleigh() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittancesForAerosol() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
AtmosphericRadiativeTerms::DataVectorType AtmosphericRadiativeTerms::GetWavelengthSpectralBand() const
{
  DataVectorType vect(m_Values.size(), 0);
  for (unsigned int nbChannel = 0; nbChannel < m_Values.size(); ++nbChannel)
  {
...
double AtmosphericRadiativeTerms::GetIntrinsicAtmosphericReflectance(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetIntrinsicAtmosphericReflectance();
}
double AtmosphericRadiativeTerms::GetSphericalAlbedo(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetSphericalAlbedo();
}
double AtmosphericRadiativeTerms::GetTotalGaseousTransmission(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetTotalGaseousTransmission();
}
double AtmosphericRadiativeTerms::GetDownwardTransmittance(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetDownwardTransmittance();
}
double AtmosphericRadiativeTerms::GetUpwardTransmittance(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetUpwardTransmittance();
}
double AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittance(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetUpwardDiffuseTransmittance();
}
double AtmosphericRadiativeTerms::GetUpwardDirectTransmittance(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetUpwardDirectTransmittance();
}
double AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittanceForRayleigh(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetUpwardDiffuseTransmittanceForRayleigh();
}
double AtmosphericRadiativeTerms::GetUpwardDiffuseTransmittanceForAerosol(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id]->GetUpwardDiffuseTransmittanceForAerosol();
}
double AtmosphericRadiativeTerms::GetWavelengthSpectralBand(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
...
const AtmosphericRadiativeTerms::ValueType AtmosphericRadiativeTerms::GetValueByIndex(unsigned int id) const
{
  if (id >= m_Values.size())
    itkExceptionMacro(<< " band index out of bounds: " << id << " for " << m_Values.size() << " bands.");
  return m_Values[id];
...
/**PrintSelf method */
void AtmosphericRadiativeTerms::PrintSelf(std::ostream& os, itk::Indent indent) const
{
  ...
  for (unsigned int i = 0; i < m_Values.size(); ++i)
  {
    ...
    os << indent << "Wavelength           : " << m_Values[i]->GetWavelengthSpectralBand() << std::endl;
  }
}
