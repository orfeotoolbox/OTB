...
CommandLineLauncher::CommandLineLauncher() : /*m_Expression(""),*/ m_VExpression(), m_WatcherList(), m_ReportProgress(true)
{
...
bool CommandLineLauncher::Load()
{
  ...
  if (this->LoadPath() == false)
  {
    ...
    {
      std::cerr << "ERROR: At least one specified path within \"" << m_Parser->GetPathsAsString(m_VExpression) << "\" is invalid or doesn't exist."
                << std::endl;
      return false;
    }
  }
      ...
      if (paramExists)
      {
                 ...
                 type == ParameterType_InputVectorData || type == ParameterType_OutputVectorData || type == ParameterType_RAM || type == ParameterType_Bool)
        {
          ...
          if (type == ParameterType_OutputImage)
          {
            ...
            {
              std::cerr << "ERROR: Too many values for parameter -" << paramKey << " (expected 2 or 1, got " << values.size() << ")." << std::endl;
              return INVALIDNUMBEROFVALUE;
            }
          }
        }
...
void CommandLineLauncher::LinkWatchers(itk::Object* itkNotUsed(caller), const itk::EventObject& event)
{
  ...
  if (m_ReportProgress)
  {
    ...
    if (typeid(otb::Wrapper::AddProcessToWatchEvent) == typeid(event))
    {
      ...
      auto watch = new StandardOneLineFilterWatcher<>(eventToWatch->GetProcess(), eventToWatch->GetProcessDescription());
