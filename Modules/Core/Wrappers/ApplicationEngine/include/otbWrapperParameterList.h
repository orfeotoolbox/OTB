...
public:
  ...
  using Self            = ParameterList;
  using Superclass      = AbstractParameterList;

  using Pointer         = itk::SmartPointer<Self>;
  using ConstPointer    = itk::SmartPointer<const Self>;

  /** Custom types */
  using ParameterVector = std::vector<typename T::Pointer>;
...
protected:
  /** Constructor */
  using AbstractParameterList::AbstractParameterList;
...
protected:
  ...
  /** */
  template <typename L, typename Get>
  const typename L::ObjectType* GetObjectList(L const&, Get) const;
