...
unsigned int ImageRegionNonUniformMultidimensionalSplitter<VImageDimension>::GetNumberOfSplits(const RegionType& region, unsigned int requestedNumber)
{
  ...
  for (j = VImageDimension; j > 0; --j)
  {
    ...
    numberOfPiecesLeft = (unsigned int)::ceil(numberOfPiecesLeft / splitsPerDimension[j - 1]);
 ...
 */
template <unsigned int            VImageDimension>
itk::ImageRegion<VImageDimension> ImageRegionNonUniformMultidimensionalSplitter<VImageDimension>::GetSplit(unsigned int i, unsigned int numberOfPieces,
                                                                                                           const RegionType& region)
{
  ...
  for (j = 0; j < VImageDimension; ++j)
  {
    ...
    else
    {
      ...
      pixelsPerSplit[j] = (unsigned int)::ceil(regionSize[j] / (double)splits[j]);
 ...
 */
template <unsigned int VImageDimension>
void ImageRegionNonUniformMultidimensionalSplitter<VImageDimension>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
