...
HooverInstanceFilter<TLabelMap>::HooverInstanceFilter() : m_NumberOfRegionsGT(0), m_NumberOfRegionsMS(0), m_Threshold(0.8), m_UseExtendedAttributes(false)
{
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::SetGroundTruthLabelMap(const LabelMapType* gt)
{
  this->SetInput(0, gt);
}
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::SetMachineSegmentationLabelMap(const LabelMapType* ms)
{
  this->SetInput(1, ms);
}
...
template <class TLabelMap>
const TLabelMap* HooverInstanceFilter<TLabelMap>::GetGroundTruthLabelMap()
{
...
template <class TLabelMap>
TLabelMap* HooverInstanceFilter<TLabelMap>::GetMachineSegmentationLabelMap()
{
...
template <class TLabelMap>
TLabelMap* HooverInstanceFilter<TLabelMap>::GetOutputGroundTruthLabelMap()
{
...
template <class TLabelMap>
TLabelMap* HooverInstanceFilter<TLabelMap>::GetOutputMachineSegmentationLabelMap()
{
  return this->GetOutput(1);
}

template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::AllocateOutputs()
{
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::ReleaseInputs()
{
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::BeforeThreadedGenerateData()
{
  ...
  if (m_NumberOfRegionsGT != m_HooverMatrix.Rows() || m_NumberOfRegionsMS != m_HooverMatrix.Cols())
  {
    itkExceptionMacro("The given Hoover confusion matrix (" << m_HooverMatrix.Rows() << " x " << m_HooverMatrix.Cols()
                                                            << ") doesn't match with the input label maps (" << m_NumberOfRegionsGT << " x "
                                                            << m_NumberOfRegionsMS << ")");
  }
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::ThreadedProcessLabelObject(LabelObjectType* labelObject)
{
...
template <class TLabelMap>
void HooverInstanceFilter<TLabelMap>::AfterThreadedGenerateData()
{
  ...
  for (unsigned int i = 0; i < m_NumberOfRegionsGT; ++i, ++iterGT)
  {
    if (GTindices.count(i) == 0)
    {
      ...
      if (m_UseExtendedAttributes)
      {
        regionGT->SetAttribute(GetNameFromAttribute(ATTRIBUTE_M), static_cast<AttributesValueType>(regionGT->GetLabel()));
      }
    }
  }
  ...
  for (unsigned int i = 0; i < m_NumberOfRegionsMS; ++i, ++iterMS)
  {
    if (MSindices.count(i) == 0)
    {
      ...
      if (m_UseExtendedAttributes)
      {
        regionMS->SetAttribute(GetNameFromAttribute(ATTRIBUTE_N), static_cast<AttributesValueType>(regionMS->GetLabel()));
      }
    }
  }
