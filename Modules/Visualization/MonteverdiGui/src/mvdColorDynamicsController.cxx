...
/*****************************************************************************/
ColorDynamicsController::ColorDynamicsController(ColorDynamicsWidget* widget, QObject* p) : AbstractModelController(widget, p)
{
}

/*****************************************************************************/
ColorDynamicsController::~ColorDynamicsController()
{
}

/*****************************************************************************/
void ColorDynamicsController::Connect(AbstractModel* model)
{
  ColorDynamicsWidget* colorDynamicsWidget = GetWidget<ColorDynamicsWidget>();

  //
  // Connect GUI to controller.
  QObject::connect(colorDynamicsWidget, SIGNAL(LowQuantileChanged(RgbwChannel, double)),
                   // to:
                   this, SLOT(OnLowQuantileChanged(RgbwChannel, double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(HighQuantileChanged(RgbwChannel, double)),
                   // to:
                   this, SLOT(OnHighQuantileChanged(RgbwChannel, double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(LowIntensityChanged(RgbwChannel, double)),
                   // to:
                   this, SLOT(OnLowIntensityChanged(RgbwChannel, double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(HighIntensityChanged(RgbwChannel, double)),
                   // to:
                   this, SLOT(OnHighIntensityChanged(RgbwChannel, double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(ResetQuantileClicked(RgbwChannel)),
                   // to:
                   this, SLOT(OnResetQuantileClicked(RgbwChannel)));

  QObject::connect(colorDynamicsWidget, SIGNAL(ResetIntensityClicked(RgbwChannel)),
                   // to:
                   this, SLOT(OnResetIntensityClicked(RgbwChannel)));

  QObject::connect(colorDynamicsWidget, SIGNAL(ApplyAllClicked(RgbwChannel, double, double)),
                   // to:
                   this, SLOT(OnApplyAllClicked(RgbwChannel, double, double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(LinkToggled(RgbwChannel, bool)),
                   // to:
                   this, SLOT(OnLinkToggled(RgbwChannel, bool)));

  QObject::connect(colorDynamicsWidget, SIGNAL(NoDataFlagToggled(bool)),
                   // to:
                   this, SLOT(OnNoDataFlagToggled(bool)));

  QObject::connect(colorDynamicsWidget, SIGNAL(NoDataValueChanged(double)),
                   // to:
                   this, SLOT(OnNoDataValueChanged(double)));

  QObject::connect(colorDynamicsWidget, SIGNAL(NoDataButtonPressed()),
                   // to:
                   this, SLOT(RefreshHistogram()));

  QObject::connect(colorDynamicsWidget, SIGNAL(GammaValueChanged(double)),
                   // to:
                   this, SLOT(OnGammaValueChanged(double)));
...
/*****************************************************************************/
void ColorDynamicsController::virtual_ResetWidget(bool isSettingModel)
{
  //
...
/*****************************************************************************/
void ColorDynamicsController::ClearWidget()
{
  ...
  bool thisSignalsBlocked = this->blockSignals(true);
  {
    for (CountType i = begin; i < end; ++i)
    {
      ...
      ColorBandDynamicsWidget* colorBandDynWgt = widget->GetChannel(channel);
...
/*****************************************************************************/
void ColorDynamicsController::Disconnect(AbstractModel* model)
{
  ColorDynamicsWidget* colorDynamicsWidget = GetWidget<ColorDynamicsWidget>();
  ...
  QObject::disconnect(colorDynamicsWidget, SIGNAL(LowQuantileChanged(RgbwChannel, double)),
                      // from:
                      this, SLOT(OnLowQuantileChanged(RgbwChannel, double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(HighQuantileChanged(RgbwChannel, double)),
                      // from:
                      this, SLOT(OnHighQuantileChanged(RgbwChannel, double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(LowIntensityChanged(RgbwChannel, double)),
                      // from:
                      this, SLOT(OnLowIntensityChanged(RgbwChannel, double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(HighIntensityChanged(RgbwChannel, double)),
                      // from:
                      this, SLOT(OnHighIntensityChanged(RgbwChannel, double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(ResetQuantileClicked(RgbwChannel)),
                      // from:
                      this, SLOT(OnResetQuantileClicked(RgbwChannel)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(ResetIntensityClicked(RgbwChannel)),
                      // from:
                      this, SLOT(OnResetIntensityClicked(RgbwChannel)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(ApplyAllClicked(RgbwChannel, double, double)),
                      // from:
                      this, SLOT(OnApplyAllClicked(RgbwChannel, double, double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(LinkToggled(RgbwChannel, bool)),
                      // to:
                      this, SLOT(OnLinkToggled(RgbwChannel, bool)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(NoDataFlagToggled(bool)),
                      // to:
                      this, SLOT(OnNoDataFlagToggled(bool)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(NoDataValueChanged(double)),
                      // to:
                      this, SLOT(OnNoDataValueChanged(double)));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(NoDataButtonPressed()),
                      // to:
                      this, SLOT(RefreshHistogram()));

  QObject::disconnect(colorDynamicsWidget, SIGNAL(GammaValueChanged(double)),
                      // to:
                      this, SLOT(OnGammaValueChanged(double)));
}

/*****************************************************************************/
void ColorDynamicsController::ResetIntensityRanges(RgbwChannel channels)
{
  // Trace.
  // qDebug()
  //   << this
  ...
  bool thisSignalsBlocked = this->blockSignals(true);
  {
    for (CountType i = begin; i < end; ++i)
    {
      ...
      ColorBandDynamicsWidget* colorBandDynWgt = colorDynamicsWidget->GetChannel(channel);

      assert(std::numeric_limits<DefaultImageType::PixelType::ValueType>::has_infinity);

      DefaultImageType::PixelType::ValueType min(-std::numeric_limits<DefaultImageType::PixelType::ValueType>::infinity());
      DefaultImageType::PixelType::ValueType max(+std::numeric_limits<DefaultImageType::PixelType::ValueType>::infinity());
...
/*****************************************************************************/
void ColorDynamicsController::ResetIntensities(RgbwChannel channels)
{
  //
  // Calculate loop bounds. Return if nothing to do.
  CountType begin = -1;
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    DefaultImageType::PixelType::ValueType min(minPx[settings.GetRgbwChannel(channel)]);

    DefaultImageType::PixelType::ValueType max(maxPx[settings.GetRgbwChannel(channel)]);
...
/*****************************************************************************/
void ColorDynamicsController::SetIntensities(RgbwChannel channels)
{
  //
  // Calculate loop bounds. Return if nothing to do.
  CountType begin = -1;
...
/*****************************************************************************/
void ColorDynamicsController::SetNoData()
{
...
/*****************************************************************************/
void ColorDynamicsController::SetGamma()
{
  assert(GetModel() == GetModel<VectorImageModel>());
  const VectorImageModel* imageModel = GetModel<VectorImageModel>();
...
/*****************************************************************************/
void ColorDynamicsController::SetBoundsEnabled(RgbwChannel channels, bool isSettingModel)
{
  // qDebug()
  //   << this
  //   << "::SetBoundsEnabled(" << channels << "," << isSettingModel << ")";
  ...
  bool hasValidHistogram = histogramModel->IsValid() && !histogramModel->IsMonoValue();
  ...
  bool thisSignalsBlocked = this->blockSignals(true);
  {
    ...
    for (CountType i = begin; i < end; ++i)
    {
      ...
      bool widgetSignalsBlocked = colorBandDynWgt->blockSignals(true);
      {
        ...
        //
        // {
        colorBandDynWgt->SetBounded((!isSettingModel ? colorBandDynWgt->IsBounded() : true) && hasValidHistogram);
        // }
...
/*****************************************************************************/
void ColorDynamicsController::ResetQuantiles(RgbwChannel channels)
{
  //
  // Calculate loop bounds. Return if nothing to do.
  CountType begin = -1;
...
/*****************************************************************************/
void ColorDynamicsController::RestoreQuantiles(RgbwChannel channels)
{
  //
  // Calculate loop bounds. Return if nothing to do.
  CountType begin = -1;
...
/*****************************************************************************/
void ColorDynamicsController::RefreshHistogram()
{
  // Trace.
  // qDebug()
  //   << this
...
/* SLOTS                                                                     */
/*****************************************************************************/
void ColorDynamicsController::OnRgbChannelIndexChanged(RgbwChannel channel, int)
{
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnGrayChannelIndexChanged(int)
{
  // qDebug()
  //   << this
...
/*******************************************************************************/
void ColorDynamicsController::OnGrayscaleActivated(bool activated)
{
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnLowQuantileChanged(RgbwChannel channel, double value)
{
  // qDebug()
  //   << this
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    HistogramModel::MeasurementType intensity = histogramModel->Quantile(settings.GetRgbwChannel(chan), 0.01 * value, BOUND_LOWER);
...
/*****************************************************************************/
void ColorDynamicsController::OnHighQuantileChanged(RgbwChannel channel, double value)
{
  // qDebug()
  //   << this
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    // Calculate quantile intensity.
    HistogramModel::MeasurementType intensity = histogramModel->Quantile(settings.GetRgbwChannel(chan), 0.01 * value, BOUND_UPPER);
...
/*****************************************************************************/
void ColorDynamicsController::OnLowIntensityChanged(RgbwChannel channel, double value)
{
  // qDebug()
  //   << this
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    if (colorBandDynWgt->IsBounded() && histogramModel->IsValid())
    {
      ...
      bool wgtSignalsBlocked = colorBandDynWgt->blockSignals(true);
      {
        ...
        colorBandDynWgt->SetLowQuantile(100.0 * histogramModel->Percentile(imageModel->GetSettings().GetRgbwChannel(chan), value, BOUND_LOWER));
      }
...
/*****************************************************************************/
void ColorDynamicsController::OnHighIntensityChanged(RgbwChannel channel, double value)
{
  // qDebug()
  //   << this
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    if (colorBandDynWgt->IsBounded() && histogramModel->IsValid())
    {
      ...
      {
        // Refresh quantile display.
        colorBandDynWgt->SetHighQuantile(100.0 *
                                         imageModel->GetHistogramModel()->Percentile(imageModel->GetSettings().GetRgbwChannel(chan), value, BOUND_UPPER));
      }
      colorBandDynWgt->blockSignals(widgetSignalsBlocked);
    }
...
/*****************************************************************************/
void ColorDynamicsController::OnResetIntensityClicked(RgbwChannel channel)
{
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnResetQuantileClicked(RgbwChannel channel)
{
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnApplyAllClicked(RgbwChannel, double low, double high)
{
  // qDebug()
  //   << this
  //   << "::OnApplyAllChanged("
  ...
  if (!RgbBounds(begin, end, settings.IsGrayscaleActivated() ? RGBW_CHANNEL_WHITE : RGBW_CHANNEL_RGB))
    return;
  ...
  for (CountType i = begin; i < end; ++i)
  {
    ...
    HistogramModel::MeasurementType lintensity = histogramModel->Quantile(band, 0.01 * low, BOUND_LOWER);

    // Calculate quantile intensity.
    HistogramModel::MeasurementType uintensity = histogramModel->Quantile(band, 0.01 * high, BOUND_UPPER);
...
/*****************************************************************************/
void ColorDynamicsController::OnNoDataFlagToggled(bool enabled)
{
  // Trace.
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnNoDataValueChanged(double value)
{
  // Trace.
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnLinkToggled(RgbwChannel channel, bool checked)
{
  // Trace.
  // qDebug()
  //   << this
...
/*****************************************************************************/
void ColorDynamicsController::OnGammaValueChanged(double value)
{
  // qDebug()
  //   << this
  //   << "::OnGammaValueChanged("
