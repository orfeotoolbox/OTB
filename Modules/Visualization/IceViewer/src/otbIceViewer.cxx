...
void IceViewer::AddVector(const std::string& fname, const std::string& key, const std::string& name)
{
  ...
  if (layers.size() > 1)
  {
    for (std::vector<std::string>::const_iterator it = layers.begin() + 1; it != layers.end(); ++it)
    {
...
void IceViewer::DrawHud()
{
  ...
  oss << "Viewport: x:[" << ulx << ", " << lrx << "], y:[" << uly << ", " << lry << "], spacing:(" << m_View->GetSettings()->GetSpacing()[0] << ", "
      << m_View->GetSettings()->GetSpacing()[1] << ")"
      << ", orientation: " << (static_cast<float>(static_cast<int>((m_View->GetSettings()->GetRotationAngle() * 1800 / M_PI)) % 3600)) / 10 << " deg"
      << std::endl;
  oss << std::endl;

  for (std::vector<std::string>::iterator it = renderingOrder.begin(); it != renderingOrder.end(); ++it)
  {
    ...
    if (currentActor.IsNotNull())
    {
      ...
      oss << "    Rendering: "
          << "R=" << currentActor->GetRedIdx() << ":[" << imageSettings->GetMinRed() << ", " << imageSettings->GetMaxRed()
          << "] G=" << currentActor->GetGreenIdx() << ":[" << imageSettings->GetMinGreen() << ", " << imageSettings->GetMaxGreen()
          << "] B=" << currentActor->GetBlueIdx() << ":[" << imageSettings->GetMinBlue() << ", " << imageSettings->GetMaxBlue() << "]"
          << ", gam=" << imageSettings->GetGamma() << ", al=" << imageSettings->GetAlpha() << ", nodata=";

      imageSettings->GetUseNoData() ? oss << "y" : oss << "n";

      oss << "\n";
      oss << "    Shader: ";

      otb::StandardShader::Pointer shader(dynamic_cast<otb::StandardShader*>(currentActor->GetShader()));
      ...
      if (!shader.IsNull())
      {
        ...
        case SHADER_LOCAL_CONTRAST:
          oss << " local contrast - radius=" << shader->GetRadius() << ", range=[" << -shader->GetLocalContrastRange() << ", "
              << shader->GetLocalContrastRange() << "]";
          break;
    ...
    else if (currentVectorActor.IsNotNull())
    {
      ...
      oss << "    Fill: " << (currentVectorActor->GetFill() ? "On" : "Off") << ", Solid border: " << (currentVectorActor->GetSolidBorder() ? "On" : "Off")
          << ", al=" << currentVectorActor->GetAlpha() << ", lw=" << currentVectorActor->GetLineWidth() << "\n";
      ...
      oss << ", optimized rendering (large vector): " << (currentVectorActor->GetOptimizedRendering() ? "On" : "Off") << " ("
          << (currentVectorActor->GetOptimizedRenderingActive() ? "active" : "inactive") << ")";
  ...
  for (std::string::iterator sit = hud_string.begin(); sit != hud_string.end(); ++sit)
  {
    if ((*sit) == '\n')
    {
      nbline += 1;
...
void IceViewer::DrawHelp()
{
  ...
  oss << "- Note that all images are reprojected in the first loaded dataset geometry (highligthed with * *). To change reference geometry to selected "
         "dataset, press P key."
      << std::endl;
  oss << std::endl;
  ...
  oss << "- Switch to local contrast enhancement with L key. In this mode, use left SHIFT + mouse wheel to adjust gain and left ALT + mouse wheel to adjust "
         "circle radius"
      << std::endl;
  oss << "- Switch to local transparency with U key. In this mode, use right SHIFT + mouse wheel to adjust gain and left ALT + mouse wheel to adjust circle "
         "radius."
      << std::endl;
  ...
  oss << "- Switch to vertical swipe transparency with S key. Press S again to change to horizontal swipe transparency. In this mode, hoover with mouse to "
         "swipe."
      << std::endl;
  oss << "- Switch to local spectral angle with T. In this mode, the greylevel value is 1 - the spectral angle with the current pixel (note that if same band "
  ...
  oss << "- Enable/disable optimized rendering for large vector (simplification of geometries, geometries smaller than 100 * viewport pixel sizes are not "
         "rendered)."
      << std::endl
      << "  Optimized rendering is only active if there are more feature to render than pixels in in the viewport." << std::endl;
  ...
  for (std::string::iterator sit = help_string.begin(); sit != help_string.end(); ++sit)
  {
    if ((*sit) == '\n')
    {
      nbline += 1;
...
void IceViewer::UpdateShaderColorAndPosition()
{
  ...
  for (std::vector<std::string>::iterator it = renderingOrder.begin(); it != renderingOrder.end(); ++it)
  {
    ...
    if (currentActor.IsNotNull())
    {
      otb::StandardShader::Pointer currentShader(dynamic_cast<otb::StandardShader*>(currentActor->GetShader()));
...
void IceViewer::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_Z && action == GLFW_PRESS)
  {
    ...
    if (m_View->ZoomToLayer(m_SelectedActor, m_View->GetSettings()->GetSpacing(), center, spacing))
    {
      m_View->GetSettings()->SetSpacing(spacing);
      m_View->GetSettings()->Center(center);
    }
  ...
  if (key == GLFW_KEY_Q && action == GLFW_PRESS)
  {
    ...
    if (m_View->ZoomToExtent(m_View->GetSettings()->GetSpacing(), center, spacing))
    {
      m_View->GetSettings()->SetSpacing(spacing);
      m_View->GetSettings()->Center(center);
    }
...
bool IceViewer::key_callback_image(GLFWwindow* window, int key, int, int action, int)
{
  ...
  otb::StandardShader::Pointer shader(dynamic_cast<otb::StandardShader*>(actor->GetShader()));

  if (shader.IsNull())
    return false;


  ImageSettings::Pointer imageSettings(actor->GetImageSettings());
  ...
  if (key == GLFW_KEY_E && action == GLFW_PRESS)
  {
    ...
    for (std::vector<std::string>::const_iterator it = actorsKeys.begin(); it != actorsKeys.end(); ++it)
    {
...
void IceViewer::CopyActorStyle(otb::GlActor::Pointer srcActor, otb::GlActor::Pointer dstActor)
{
  ...
  if (srcImgActor.IsNotNull() && srcActor->GetVisible() && dstImgActor.IsNotNull() && dstActor->GetVisible())
  {
    ...
    if (srcShader.IsNotNull() && dstShader.IsNotNull())
    {
      ...
      if (srcShader->GetShaderType() == SHADER_STANDARD || srcShader->GetShaderType() == SHADER_LOCAL_CONTRAST ||
          srcShader->GetShaderType() == SHADER_SPECTRAL_ANGLE || srcShader->GetShaderType() == SHADER_GRADIENT)
      {
