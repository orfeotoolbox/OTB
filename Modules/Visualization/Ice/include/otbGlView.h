...
{
  assert(!std::numeric_limits<T>::has_quiet_NaN || (std::numeric_limits<T>::has_quiet_NaN && val != std::numeric_limits<T>::quiet_NaN()));
...
{
  assert(!std::numeric_limits<T>::has_signaling_NaN || (std::numeric_limits<T>::has_signaling_NaN && val != std::numeric_limits<T>::signaling_NaN()));
  // make production build happy
  ((void)val);
}
 ...
 */
class OTBIce_EXPORT GlView : public itk::Object
{
public:
  typedef GlView                        Self;
...
template <typename P, typename S, typename P2, typename S2>
bool GlView::ReprojectFromView(P& center, S& spacing, const KeyType& key, const P2& vcenter, const S2& vspacing, double norm) const
{
  ...
  const otb::GeoInterface* geo = dynamic_cast<const GeoInterface*>(actor.GetPointer());
...
template <typename P>
size_t GlView::GetExtent(P& origin, P& extent, bool isOverlay) const
{
  if (m_Actors.empty())
  {
    origin.Fill(0);
...
template <typename Point, typename Spacing>
bool GlView::ZoomToExtent(const Spacing& native, Point& center, Spacing& spacing) const
{
  Point o;
  Point e;
...
template <typename Point, typename Spacing>
bool GlView::ZoomToLayer(const KeyType& key, const Spacing& native, Point& center, Spacing& spacing) const
{
  Point o;
  Point e;
...
template <typename Point, typename Spacing>
bool GlView::ZoomToRegion(const Point& origin, const Point& extent, const Spacing& native, Point& center, Spacing& spacing) const
{
  // std::cout
...
template <typename Point, typename Spacing>
bool GlView::ZoomToFull(const KeyType& key, Point& center, Spacing& spacing, double units) const
{
  // std::cout
  //   << std::hex << this << std::dec
  //   << "::ZoomToFull();"
  ...
  spacing[0] = std::abs(n_spacing[0]) * units * spacing[0] / std::sqrt(e[0] * e[0] + e[1] * e[1]);
  ...
  // MANTIS-1203: absolute value of native spacing should be
  // considered (to avoid flipping effects).
  spacing[1] = std::abs(n_spacing[1]) * units * spacing[1] / std::sqrt(e[0] * e[0] + e[1] * e[1]);
