...
void GlImageActor::CreateShader()
{
...
const GeoInterface::Spacing2& GlImageActor::GetSpacing() const
{
  return m_Spacing;
}
...
bool GlImageActor::HasKwl() const
{
  return true;
}


bool GlImageActor::GetKwl(ImageKeywordlist& kwl) const
{
...
void GlImageActor::UpdateData()
{
  ...
  unsigned int nbTilesX = std::ceil(static_cast<double>(requested.GetIndex()[0] + requested.GetSize()[0]) / m_TileSize) -
                          std::floor(static_cast<double>(requested.GetIndex()[0]) / m_TileSize);
  unsigned int nbTilesY = std::ceil(static_cast<double>(requested.GetIndex()[1] + requested.GetSize()[1]) / m_TileSize) -
                          std::floor(static_cast<double>(requested.GetIndex()[1]) / m_TileSize);
...
bool GlImageActor::TileAlreadyLoaded(const Tile& tile)
{
  for (TileVectorType::iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
  {
    ...
    {
      return (tile.m_Resolution == it->m_Resolution && tile.m_RedIdx == it->m_RedIdx && tile.m_GreenIdx == it->m_GreenIdx && tile.m_BlueIdx == it->m_BlueIdx);
    }
  }
...
void GlImageActor::Render()
{
  ...
  else
  {
    ...
    for (TileVectorType::iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
    {
      ...
      gamma = gamma == 0.0 ? std::numeric_limits<double>::max() : 1.0 / gamma;
      // }
      ...
      // #endif
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, it->m_RescaleFilter->GetOutput()->GetLargestPossibleRegion().GetSize()[0],
                   it->m_RescaleFilter->GetOutput()->GetLargestPossibleRegion().GetSize()[1], 0, GL_BGRA, GL_UNSIGNED_BYTE, buffer);
...
void GlImageActor::LoadTile(Tile& tile)
{
  ...
  if (!m_SoftwareRendering)
  {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
    // #endif
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, extract->GetOutput()->GetLargestPossibleRegion().GetSize()[0],
                 extract->GetOutput()->GetLargestPossibleRegion().GetSize()[1], 0, GL_BGRA, GL_FLOAT, buffer);
...
void GlImageActor::CleanLoadedTiles()
{
  ...
  for (TileVectorType::iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
  {
    ...
    if (!tileRegion.Crop(requested) || it->m_Resolution != m_CurrentResolution || it->m_RedIdx != m_RedIdx || it->m_GreenIdx != m_GreenIdx ||
...
bool GlImageActor::GetPixelFromViewport(const PointType& ptView, PixelType& pixel, PointType& ptPhysical, IndexType& index) const
{
...
bool GlImageActor::GetPixel(const PointType& physical, PixelType& pixel, IndexType& index) const
{
...
// First, we need to return index in full img (not in overviews)
...
#else
  index[0] = static_cast<IndexType::IndexValueType>((physical[0] + 0.5 * m_Spacing[0] - m_Origin[0]) / m_Spacing[0]);

  index[1] = static_cast<IndexType::IndexValueType>((physical[1] + 0.5 * m_Spacing[1] - m_Origin[1]) / m_Spacing[1]);
  ...
  for (TileVectorType::const_iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
  {
    // std::cout
    //   << "R: ("
...
void GlImageActor::UpdateResolution()
{
  ...
  if (resolution < 1.0)
  {
    ...
    for (ResolutionVectorType::iterator it = m_AvailableResolutions.begin(); it != m_AvailableResolutions.end(); ++it)
    {
      ...
      if (((m_ResolutionAlgorithm == ResolutionAlgorithm::Nearest_Lower && diff <= 0) ||
           (m_ResolutionAlgorithm == ResolutionAlgorithm::Nearest_Upper && diff >= 0) || (m_ResolutionAlgorithm == ResolutionAlgorithm::Nearest)) &&
...
void GlImageActor::UpdateTransforms()
{
  ...
  if ((!settings->GetUseProjection() && geometryChanged) || m_ViewportToImageTransform.IsNull() || m_ImageToViewportTransform.IsNull())
  {
    // std::cout
  ...
  geometryChanged = geometryChanged || (m_ViewportToImageTransform.IsNotNull() && m_ViewportToImageTransform->GetInputProjectionRef() != settings->GetWkt()) ||
...
void GlImageActor::AutoColorAdjustment(double& minRed, double& maxRed, double& minGreen, double& maxGreen, double& minBlue, double& maxBlue, bool full,
                                       unsigned int refSize, double lcp, double hcp)
{
  ...
  if (full)
  {
    ...
    for (ResolutionVectorType::iterator it = m_AvailableResolutions.begin(); it != m_AvailableResolutions.end() && !found; ++it, ++resol)
    {
...
bool GlImageActor::TransformFromViewport(Point2d& out, const Point2d& in, bool isPhysical) const
{
...
bool GlImageActor::TransformToViewport(Point2d& out, const Point2d& in, bool isPhysical) const
{
