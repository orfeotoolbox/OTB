...
GlView::GlView() : m_Settings(ViewSettings::New()), m_Actors(), m_RenderingOrder()
{
}
...
bool GlView::ContainsActor(const KeyType& key) const
{
...
std::vector<std::string> GlView::GetActorsKeys() const
{
  ...
  for (ActorMapType::const_iterator it = m_Actors.begin(); it != m_Actors.end(); ++it)
  {
...
void GlView::LightRender()
{
  ...
  for (StringVectorType::reverse_iterator it = m_RenderingOrder.rbegin(); it != m_RenderingOrder.rend(); ++it)
  {
...
void GlView::SetRenderingOrder(const StringVectorType& keys, bool front)
{
  StringVectorType thisKeys(m_RenderingOrder);

  m_RenderingOrder.clear();

  for (StringVectorType::const_iterator it(keys.begin()); it != keys.end(); ++it)
    if (ContainsActor(*it))
    {
  ...
  m_RenderingOrder.insert(front ? m_RenderingOrder.end() : m_RenderingOrder.begin(), thisKeys.begin(), thisKeys.end());
}


void GlView::SaveScreenshot(const std::string& filename) const
{
  //
  // Check input(s).
  ...
  if (glGetError() != GL_NO_ERROR)
    throw std::runtime_error(std::string(reinterpret_cast<const char*>(gluErrorString(glGetError()))));
  ...
  glReadPixels(0, 0, size[0], size[1], GL_RGB, GL_UNSIGNED_BYTE, glBuffer);
  ...
  filter->SetImportPointer(itkBuffer, count, true);
