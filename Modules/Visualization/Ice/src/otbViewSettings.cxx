...
namespace otb
{
ViewSettings::ViewSettings() : m_Wkt(""), m_RotationAngle(0.0), m_UseProjection(true), m_GeometryChanged(false)
{
  m_Origin.Fill(0);
  m_Spacing.Fill(1);
...
double ViewSettings::GetScale(const PointType& origin, const PointType& extent, bool isSigned) const
{
  SpacingType spacing;
  ...
  if (scale[0] > scale[1])
    return isSigned ? spacing[0] : scale[0];

  return isSigned ? spacing[1] : scale[1];
}
