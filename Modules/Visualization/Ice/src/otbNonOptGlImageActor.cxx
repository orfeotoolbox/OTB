...
bool NonOptGlImageActor::TileAlreadyLoaded(const Tile& tile)
{
  for (TileVectorType::iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
  {
    ...
    {
      bool resp = (tile.m_Resolution == it->m_Resolution && tile.m_RedIdx == it->m_RedIdx && tile.m_GreenIdx == it->m_GreenIdx &&
                   tile.m_BlueIdx == it->m_BlueIdx && (tile.m_UseShader == m_UseShader));

      if (!m_UseShader)
      {
        return resp && tile.m_MinRed == m_MinRed && tile.m_MinGreen == m_MinGreen && tile.m_MinBlue == m_MinBlue && tile.m_MaxRed == m_MaxRed &&
               tile.m_MaxGreen == m_MaxGreen && tile.m_MaxBlue == m_MaxBlue;
      }
      else
      {
        return resp;
...
void NonOptGlImageActor::LoadTile(Tile& tile)
{
  ...
  if (!tile.m_UseShader)
  {
    ...
    // #endif
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, rescale->GetOutput()->GetLargestPossibleRegion().GetSize()[0],
                 rescale->GetOutput()->GetLargestPossibleRegion().GetSize()[1], 0, GL_BGRA, GL_UNSIGNED_BYTE, buffer);
...
void NonOptGlImageActor::CleanLoadedTiles()
{
  ...
  for (TileVectorType::iterator it = m_LoadedTiles.begin(); it != m_LoadedTiles.end(); ++it)
  {
    ...
    if (!tileRegion.Crop(requested) || it->m_Resolution != m_CurrentResolution || it->m_RedIdx != m_RedIdx || it->m_GreenIdx != m_GreenIdx ||
        it->m_BlueIdx != m_BlueIdx || (it->m_UseShader != m_UseShader) || (!m_UseShader && it->m_MinRed != m_MinRed) ||
        (!m_UseShader && it->m_MinGreen != m_MinGreen) || (!m_UseShader && it->m_MinBlue != m_MinBlue) || (!m_UseShader && it->m_MaxRed != m_MaxRed) ||
        (!m_UseShader && it->m_MaxGreen != m_MaxGreen) || (!m_UseShader && it->m_MaxBlue != m_MaxBlue))
    {
...
void NonOptGlImageActor::UpdateResolution()
{
  ...
  double resolution = std::min(std::abs(m_Spacing[0] / outSpacing[0]), std::abs(m_Spacing[1] / outSpacing[1]));
