...
static void ICE_CALLBACK TesselationCombineCallback(GLdouble coords[3], GLdouble * /*data*/ [4], GLfloat /*weights*/[4], GLdouble** dataOut)
{
...
GlVectorActor::PointType GlVectorActor::ViewportToVectorTransform(const PointType& vpPoint) const
{
  /*
...
void GlVectorActor::UpdateData()
{
  ...
  if (m_ExtentULX != ulx || m_ExtentULY != uly || m_ExtentLRX != lrx || m_ExtentLRY != lry || m_InternalFeatures.empty())
  {
    ...
    double areaOfScreenPixel = std::abs(lrx - ulx) * std::abs(lry - uly) / (settings->GetViewportSize()[0] * settings->GetViewportSize()[1]);
    ...
    m_OptimizedRenderingActive =
        m_OptimizedRendering && filtered.GetFeatureCount(true) > static_cast<int>(settings->GetViewportSize()[0] * settings->GetViewportSize()[1] / 100);
...
void GlVectorActor::UpdateDisplayList()
{
  ...
  for (std::vector<InternalFeature>::iterator it = m_InternalFeatures.begin(); it != m_InternalFeatures.end(); ++it)
  {
...
bool GlVectorActor::TransformFromViewport(Point2d& out, const Point2d& in, bool isPhysical) const
{
  if (!isPhysical)
    return false;
