...
std::string StandardShader::GetSource() const
{
  ...
  {
    itkExceptionMacro(<< " Required GL and GLSL versions were not found (GL version is " << glVersion << ", should be at least "
                      << otb::GlVersionChecker::REQUIRED_GL_VERSION << ", GLSL version is " << glslVersion << ", should be at least "
                      << otb::GlVersionChecker::REQUIRED_GLSL_VERSION << ")");
  }
  ...
  shader_source +=
      ...
      "{\n"
      "float alpha = (mod(floor(gl_FragCoord.x / shader_chessboard_size), 2.0) == 0.) != (mod(floor(gl_FragCoord.y / shader_chessboard_size), 2.0) == 1.) ? "
      "shader_alpha : 0.0;\n"
      "gl_FragColor[3] = clamp(alpha,0.0,1.0);\n"
      "}\n"
      "else if(shader_type == 4)\n"
      "{\n"
      "float alpha = (shader_vertical_slider_flag == 0 && gl_FragCoord.x > shader_slider_pos) || (shader_vertical_slider_flag == 1 && gl_FragCoord.y > "
...
void StandardShader::SetupShader()
{
  ...
  glUniform3f(shader_current, m_ImageSettings->GetCurrentRed(), m_ImageSettings->GetCurrentGreen(), m_ImageSettings->GetCurrentBlue());
  ...
  GLint shader_spectral_angle_range =
      glGetUniformLocation(otb::FragmentShaderRegistry::Instance()->GetShaderProgram("StandardShader"), "shader_spectral_angle_range");
  ...
  GLint shader_vertical_slider_flag =
      glGetUniformLocation(otb::FragmentShaderRegistry::Instance()->GetShaderProgram("StandardShader"), "shader_vertical_slider_flag");
