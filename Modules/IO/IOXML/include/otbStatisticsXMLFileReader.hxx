...
StatisticsXMLFileReader<TMeasurementVector>::StatisticsXMLFileReader() : m_FileName(""), m_IsUpdated(false)
{
}

template <class TMeasurementVector>
unsigned int StatisticsXMLFileReader<TMeasurementVector>::GetNumberOfOutputs()
{
  return m_MeasurementVectorContainer.size() + m_GenericMapContainer.size();
}

template <class TMeasurementVector>
std::vector<std::string> StatisticsXMLFileReader<TMeasurementVector>::GetStatisticVectorNames()
{
...
template <class TMeasurementVector>
std::vector<std::string> StatisticsXMLFileReader<TMeasurementVector>::GetStatisticMapNames()
{
  // Read the xml file once
  if (!m_IsUpdated)
  {
  ...
  std::vector<std::string> output;
  for (GenericMapContainer::iterator it = m_GenericMapContainer.begin(); it != m_GenericMapContainer.end(); ++it)
  {
...
template <class TMeasurementVector>
typename StatisticsXMLFileReader<TMeasurementVector>::MeasurementVectorType
StatisticsXMLFileReader<TMeasurementVector>::GetStatisticVectorByName(const char* statisticName)
{
  // Read the xml file once
  if (!m_IsUpdated)
  {
...
MapType StatisticsXMLFileReader<TMeasurementVector>::GetStatisticMapByName(const char* statisticName)
{
  // Read the xml file once
  if (!m_IsUpdated)
  {
  ...
  for (GenericMapType::iterator it = m_GenericMapContainer[statName].begin(); it != m_GenericMapContainer[statName].end(); ++it)
  {
...
template <class TMeasurementVector>
void StatisticsXMLFileReader<TMeasurementVector>::Read()
{
  ...
  {
    itkExceptionMacro(<< extension << " is a wrong Extension FileName : Expected .xml");
  }
  ...
  if (root)
  {
    ...
    for (TiXmlElement* currentStat = root->FirstChildElement(); currentStat != nullptr; currentStat = currentStat->NextSiblingElement())
    {
      ...
      for (TiXmlElement* sample = currentStat->FirstChildElement("StatisticVector"); sample != nullptr; sample = sample->NextSiblingElement())
      {
      ...
      for (unsigned int i = 0; i < tempMeasurementVector.size(); ++i)
        currentStatisticVector.second.SetElement(i, (static_cast<InputValueType>(tempMeasurementVector[i])));
  ...
  if (root)
  {
    ...
    for (TiXmlElement* currentStat = root->FirstChildElement(); currentStat != nullptr; currentStat = currentStat->NextSiblingElement())
    {
      ...
      for (TiXmlElement* sample = currentStat->FirstChildElement("StatisticMap"); sample != nullptr; sample = sample->NextSiblingElement())
      {
...
template <class TMeasurementVector>
void StatisticsXMLFileReader<TMeasurementVector>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
  ...
  {
    if (it != m_GenericMapContainer.begin())
      os << ", ";
    os << it->first;
  }
  os << std::endl;
