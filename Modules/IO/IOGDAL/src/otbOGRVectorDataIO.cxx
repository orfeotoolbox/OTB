...
OGRVectorDataIO::OGRVectorDataIO() : m_DataSource(nullptr)
{
...
void OGRVectorDataIO::PrintSelf(std::ostream& os, itk::Indent indent) const
{
...
void OGRVectorDataIO::Read(itk::DataObject* datag)
{
  ...
  m_DataSource = (GDALDataset*)GDALOpenEx(this->m_FileName.c_str(), GDAL_OF_READONLY | GDAL_OF_VECTOR, NULL, NULL, NULL);
  if (m_DataSource == nullptr)
  {
  ...
  for (int layerIndex = 0; layerIndex < m_DataSource->GetLayerCount(); ++layerIndex)
  {
    ...
    //       // std::cout<<"Document "<<document->GetNodeId()<<": Adding field "<<field->GetNameRef()<<"
    //       "<<OGRFieldDefn::GetFieldTypeName(field->GetType())<<std::endl;
    //     }
...
void OGRVectorDataIO::Write(const itk::DataObject* datag, char** /** unused */)
{
  ...
  GDALDriver* ogrDriver = GetGDALDriverManager()->GetDriverByName(this->GetOGRDriverName(this->m_FileName).data());
  ...
  m_DataSource = ogrDriver->Create(this->m_FileName.c_str(), 0, 0, 0, GDT_Unknown, 0);
  ...
  chrono.Stop();
  otbMsgDevMacro(<< "OGRVectorDataIO: file saved in " << chrono.GetElapsedMilliseconds() << " ms. (" << layerKept << " elements)");

  otbMsgDevMacro(<< " OGRVectorDataIO::Write()  ");
}


std::string OGRVectorDataIO::GetOGRDriverName(std::string name) const
{
  ...
  else
  {
    ...
    if (m_OGRExtensionsToDrivers.count(extension) == 1)
      driverOGR = m_OGRExtensionsToDrivers.at(extension);
    else
      driverOGR = "NOT-FOUND";
  }
  return driverOGR;
