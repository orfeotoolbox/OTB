...
KmzProductWriter<TInputImage>::KmzProductWriter() : m_UseExtendMode(true), m_TileSize(512), m_MaxDepth(0), m_CurrentDepth(0), m_CurIdx(0)
{
...
template <class TInputImage>
KmzProductWriter<TInputImage>::~KmzProductWriter()
{
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::SetInput(const InputImageType* input)
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::SetInput(unsigned int index, const TInputImage* image)
{
  // Process object is not const-correct so the const_cast is required here
 ...
 */
template <class TInputImage>
const typename KmzProductWriter<TInputImage>::InputImageType* KmzProductWriter<TInputImage>::GetInput(void)
{
 ...
 */
template <class TInputImage>
const typename KmzProductWriter<TInputImage>::InputImageType* KmzProductWriter<TInputImage>::GetInput(unsigned int idx)
{
  return static_cast<const TInputImage*>(this->ProcessObject::GetInput(idx));
}

template <class TInputImage>
void KmzProductWriter<TInputImage>::Write()
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::Initialize()
{
  ...
  {
    itkExceptionMacro(<< itksys::SystemTools::GetFilenameLastExtension(m_Path) << " is a wrong Extension FileName : Expected .kmz");
  }
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::AddLogo()
{
  // Logo
  if (m_Logo.IsNull())
    return;
...
/**
  ...
  */
template <class TInputImage>
void KmzProductWriter<TInputImage>::ProcessLegends()
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::Tiling()
{
    ...
    for (unsigned int tx = 0, x = 0; tx < sizeX; tx += m_TileSize, ++x)
    {
      for (unsigned int ty = 0, y = 0; ty < sizeY; ty += m_TileSize, ++y)
      {
        ...
        else
        {
          ...
          if (!m_UseExtendMode)
          {
            this->GenerateKMLWithLink(path.str(), depth, x, y, tileXStart, tileYStart, north, south, east, west, centerLong, centerLat);
          }
          else
          {
            this->GenerateKMLExtendedWithLink(path.str(), depth, x, y, tileXStart, tileYStart, lowerLeftCorner, lowerRightCorner, upperRightCorner,
                                              upperLeftCorner, centerLong, centerLat);
          }
        }
...
*/
template <class TInputImage>
int KmzProductWriter<TInputImage>::AddFileToKMZ(const std::ostringstream& absolutePath, const std::ostringstream& kmz_in_path)
{
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateKMLRoot(const std::string& title, double north, double south, double east, double west, bool itkNotUsed(extended))
{
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::RootKmlProcess(double north, double south, double east, double west)
{
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::CloseRootKML()
{
  if (!m_Logo.IsNull())
  {
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::AddNetworkLinkToRootKML(double north, double south, double east, double west, const std::string& directory, bool addRegion,
                                                            unsigned int pos)
{
  m_RootKmlFile << "\t\t<Document>" << std::endl;
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::BoundingBoxKmlProcess(double north, double south, double east, double west)
{
 ...
 */
template <class TInputImage>
void KmzProductWriter<TInputImage>::AddLegendToRootKml(double north, double south, double east, double west)
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateKMLExtended(const std::string& pathname, int depth, int itkNotUsed(x), int y, OutputPointType lowerLeft,
                                                        OutputPointType lowerRight, OutputPointType upperRight, OutputPointType upperLeft)
{
  std::ostringstream kmlname;
  kmlname << pathname;
  kmlname << "/";
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateKML(const std::string& pathname, int depth, int itkNotUsed(x), int y, double north, double south, double east,
                                                double west)
{
  std::ostringstream kmlname;
  kmlname << pathname;
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateKMLExtendedWithLink(const std::string& pathname, int depth, int itkNotUsed(x), int y, int tileStartX,
                                                                int tileStartY, OutputPointType lowerLeft, OutputPointType lowerRight,
                                                                OutputPointType upperRight, OutputPointType upperLeft, double centerLong, double centerLat)
{
  std::ostringstream kmlname;
  kmlname << pathname;
  kmlname << "/";
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateKMLWithLink(const std::string& pathname, int depth, int itkNotUsed(x), int y, int tileStartX, int tileStartY,
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::GenerateBoundingKML(double north, double south, double east, double west)
{
  std::ostringstream kmlname;
  kmlname << m_Path;
  kmlname << "/";
  ...
  fileTest << "\t<Document>" << std::endl;
  fileTest << "\t\t<name> Bounding box of the  product " << m_FileName << "</name>" << std::endl;
 ...
 */
template <class TInputImage>
std::string KmzProductWriter<TInputImage>::GetCuttenFileName(const std::string& itkNotUsed(description), unsigned int idx)
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::PrintSelf(std::ostream& os, itk::Indent indent) const
{
...
template <class TInputImage>
void KmzProductWriter<TInputImage>::SetTileSize(unsigned int tileSize)
{
  if (tileSize > 1)
    m_TileSize = tileSize;
  else
