...
public:
  ...
  typedef otb::StereorectificationDisplacementFieldSource<FloatVectorImageType, FloatVectorImageType> DisplacementFieldSourceType;
...
private:
  ...
  void DoInit() override
  {
    ...
    SetDescription(
        "Generates two deformation fields to resample in epipolar "
    ...
    SetDocLongDescription(
        ...
        "ratio.");
    SetDocLimitations(
        "Generation of the deformation grid is not streamable, "
    ...
    SetParameterDescription("io.outright",
                            "The deformation grid to resample the"
    ...
    SetParameterDescription("epi.elevation.avgdem.mindisp",
                            "Disparity "
    ...
    SetParameterDescription("epi.elevation.avgdem.maxdisp",
                            "Disparity "
    ...
    SetParameterDescription("epi.scale",
                            "The scale parameter allows generating"
    ...
    SetParameterDescription("epi.step",
                            "Stereo-rectification deformation grid "
    ...
    SetParameterDescription("epi.rectsizex",
                            "The application computes the "
    ...
    SetParameterDescription("epi.rectsizey",
                            "The application computes the "
                            "optimal rectified image size so that the whole left input image fits "
                            "into the rectified area. However, due to the scale and step parameter, "
                            "this size may not match the size of the deformation field output. In "
    ...
    SetParameterDescription("epi.baseline",
                            "This parameter is the mean value, "
    ...
    SetParameterDescription("inverse",
                            "This group of parameter allows "
    ...
    SetParameterDescription("inverse.outleft",
                            "The deformation grid to resample"
    ...
    SetParameterDescription("inverse.outright",
                            "The output deformation grid to"
    ...
    SetParameterDescription("inverse.ssrate",
                            "Grid inversion is an heavy "
  ...
  void DoExecute() override
  {
    ...
    if (otb::Wrapper::ElevationParametersHandler::IsGeoidUsed(this, "epi.elevation") &&
        otb::Wrapper::ElevationParametersHandler::IsDEMUsed(this, "epi.elevation") && !IsParameterEnabled("epi.elevation.avgdem.step"))
    {
    ...
    if (IsParameterEnabled("epi.elevation.avgdem.step"))
    {
      ...
      SetParameterFloat("epi.elevation.avgdem.mindisp",
                        (m_StatisticsFilter->GetMinimum() - m_StatisticsFilter->GetMean()) * m_DisplacementFieldSource->GetMeanBaselineRatio());
      EnableParameter("epi.elevation.avgdem.maxdisp");
      SetParameterFloat("epi.elevation.avgdem.maxdisp",
                        (m_StatisticsFilter->GetMaximum() - m_StatisticsFilter->GetMean()) * m_DisplacementFieldSource->GetMeanBaselineRatio());
    ...
    if (IsParameterEnabled("inverse.outleft"))
    {
      ...
      if (lsize[0] * lsize[1] > 256 * 256)
        otbAppLogWARNING("Left image's size ("
                         << lsize[0] << "x" << lsize[1]
                         << ") may result in a too high computational demand. Please consider setting inverse.ssrate parameter correctly (actual value is "
                         << GetParameterInt("inverse.ssrate") << ").");
    ...
    if (IsParameterEnabled("inverse.outright"))
    {
      ...
      if (rsize[0] * rsize[1] > 256 * 256)
        otbAppLogWARNING("Right image's size ("
                         << rsize[0] << "x" << rsize[1]
                         << ") may result in a too high computational demand. Please consider setting inverse.ssrate parameter correctly (actual value is "
                         << GetParameterInt("inverse.ssrate") << ").");
