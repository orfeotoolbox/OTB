...
public:
  ...
  typedef otb::FineRegistrationImageFilter<ScalarImageType, ScalarImageType, FieldImageType> RegistrationFilterType;
  typedef itk::DiscreteGaussianImageFilter<ScalarImageType, ScalarImageType>                      GaussianFilterType;
  typedef itk::VectorIndexSelectionCastImageFilter<FieldImageType, ScalarImageType>               VectorImageToImageFilterType;
  ...
  typedef otb::VectorImageToIntensityImageFilter<FloatVectorImageType, ScalarImageType>           IntensityFilterType;
  typedef itk::NormalizedCorrelationImageToImageMetric<ScalarImageType, ScalarImageType>          NCCType;
  typedef itk::MeanSquaresImageToImageMetric<ScalarImageType, ScalarImageType>                    MeanSquareType;
  typedef itk::MeanReciprocalSquareDifferenceImageToImageMetric<ScalarImageType, ScalarImageType> MRSDType;
  typedef itk::MattesMutualInformationImageToImageMetric<ScalarImageType, ScalarImageType>        MIType;
  ...
  typedef StreamingWarpImageFilter<FloatVectorImageType, FloatVectorImageType, FieldImageType> WarpFilterType;
...
private:
  void DoInit() override
  {
    ...
    SetDocLongDescription(
        "This application computes a disparity map between "
    ...
    SetParameterDescription("out",
                            "The output image contains 3 bands, for X "
    ...
    SetParameterDescription("mrx",
                            "Radius along x (in pixels) of the metric "
                            "computation window");
    SetMinimumParameterIntValue("mrx", 0);

    AddParameter(ParameterType_Int, "mry", "Metric Radius Y");
    SetParameterDescription("mry",
                            "Radius along y (in pixels) of the metric "
                            "computation window");
    SetMinimumParameterIntValue("mry", 0);

    AddParameter(ParameterType_InputImage, "w", "Image To Warp");
    SetParameterDescription("w",
                            "The image to warp after disparity estimation"
                            " is completed.");
    MandatoryOff("w");
    ...
    SetParameterDescription("cox",
                            "Coarse offset along x (in physical space)"
    ...
    SetParameterDescription("coy",
                            "Coarse offset along y (in physical space)"
                            " between the two images, used as an initial offset for all pixels.");
    SetDefaultParameterFloat("coy", 0.0);
    MandatoryOff("coy");

    AddParameter(ParameterType_Float, "ssrx", "Sub-Sampling Rate X");
    SetParameterDescription("ssrx",
                            "Generates a result at a coarser "
    ...
    SetParameterDescription("rgsx",
                            "Performs a gaussian smoothing of the "
    ...
    SetParameterDescription("spa",
                            "Metric extrema location will be refined up"
    ...
    SetParameterDescription("cva",
                            "Metric extrema will be refined up to the"
    ...
    SetParameterDescription("vmlt",
                            "Lower threshold to compute the validity "
                            "mask. This mask will be the 4th output band.");
    MandatoryOff("vmlt");

    AddParameter(ParameterType_Float, "vmut", "Validity Mask Upper Threshold");
    SetParameterDescription("vmut",
                            "Upper threshold to obtain a validity "
                            "mask. This mask will be the 4th output band.");
    MandatoryOff("vmut");
