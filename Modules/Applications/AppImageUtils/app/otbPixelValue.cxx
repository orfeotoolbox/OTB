...
private:
  void DoInit() override
  {
    ...
    SetDocLongDescription(
        "This application gives the value of a selected "
    ...
    AddParameter(ParameterType_Choice, "mode", "Coordinate system used to designate the pixel");
    SetParameterDescription("mode", "Different modes can be selected, default mode is Index.");
    AddChoice("mode.index", "Index");
    SetParameterDescription("mode.index", "This mode uses the given coordinates as index to locate the pixel.");
  ...
  void DoExecute() override
  {
    ...
    if (mode == "index")
    {
      ...
      if (static_cast<unsigned int>(id[0]) >= inImage->GetLargestPossibleRegion().GetSize()[0] ||
          static_cast<unsigned int>(id[1]) >= inImage->GetLargestPossibleRegion().GetSize()[1] || id[0] < 0 || id[1] < 0)
      {
        id.Fill(0);
        isPixelIn = false;
      }
