...
private:
  ...
  void DoInit()
  {
    ...
    SetParameterDescription("comp.feather.large", "Blends all images on the maximum overlapping areas (this produces seamless mosaics, but can cause blur effect where images are not perfectly aligned)");

    // comp.feather.slim
    AddChoice              ("comp.feather.slim", "The slim blending composition mode");
    SetParameterDescription("comp.feather.slim", "Blends the last image over earlier ones in areas of overlap, on a given transition distance (seam can be visible from a certain zoom level, but it does not cause blur effect when images are not perfectly aligned)");
    ...
    SetParameterDescription("harmo.method.band", "Consists in minimizing a cost function based on images statistics in overlapping areas, for each band independently");
    AddChoice              ("harmo.method.rgb", "True colors");
    SetParameterDescription("harmo.method.rgb", "Consists in minimizing a cost function based on images statistics in overlapping areas, in a decorreleted color space suitable for true color processing (works only on true color images, i.e. RGB). Only the first 3 bands are processed by this method. ");
