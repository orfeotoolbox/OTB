...
private:
  void DoInit() override
  {
    ...
    oss << "This application uses inverse sensor modelling combined with a choice of interpolation functions to resample a sensor geometry image into a ground "
           "geometry regular grid. ";
    oss << "The ground geometry regular grid is defined with respect to a map projection (see map parameter). The application offers several modes to estimate "
           "the output grid parameters (origin and ground sampling distance), including automatic estimation of image size, ground sampling distance, or both, "
           "from image metadata, user-defined ROI corners, or another ortho-image.";
    ...
    SetDocLimitations(
        "Supported sensors (both optical and radar) are: GeoEye, Ikonos, Pleiades, Quickbird, RadarSat, Sentinel-1, SPOT5 (TIF format), SPOT6/7, TerraSAR-X, "
        "Worldview 1/2/3, and any TIF image with embedded RPC tags.\n Also note that the opt.gridspacing default value may not be suitable for all sensors. In "
        "particular, if this value is lower than the target ground sampling distance, the processing time may increase a lot. A warning is issued in this "
        "case. Typical values should be half the DEM ground sampling distance.");
    ...
    SetParameterDescription("outputs.mode.autosize",
    ...
    SetParameterDescription("outputs.mode.autospacing",
    ...
    SetParameterDescription("outputs.mode.outputroi",
    ...
    isotropOss
        << "Default spacing (pixel size) values are estimated from the sensor modeling of the image. It can therefore result in a non-isotropic spacing. ";
    ...
    SetParameterDescription("interpolator.bco.radius",
                            "This parameter allows one to control the size of the bicubic interpolation filter. If the target pixel size is higher than the "
                            "input pixel size, increasing this parameter will reduce aliasing artifacts.");
    ...
    SetParameterDescription(
        "opt.rpc", "Enabling RPC modeling allows one to speed-up SPOT5 ortho-rectification. Value is the number of control points per axis for RPC estimation");
    DisableParameter("opt.rpc");
    MandatoryOff("opt.rpc");

    // RAM available
    AddRAMParameter("opt.ram");
    SetParameterDescription("opt.ram",
