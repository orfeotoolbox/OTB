...
// This is ugly ...
  ...
  typedef otb::KeyPointSetsMatchingFilter<PointSetType, DistanceType> MatchingFilterType;

  typedef MatchingFilterType::LandmarkListType LandmarkListType;
  typedef PointSetType::PointType              PointType;
  typedef otb::MultiToMonoChannelExtractROI<ValueType, ValueType> ExtractChannelFilterType;
...
private:
  void DoInit() override
  {
    ...
    SetDocLongDescription(
        "This application computes homologous points between images using keypoints. "
    ...
    SetParameterDescription("mfilter",
                            "If enabled, this option allows one to filter matches according to colocalisation from sensor or geographical information, using "
                            "the given tolerancy expressed in pixels");
  ...
  void Match(FloatImageType* im1, FloatImageType* im2, RSTransformType* rsTransform, RSTransformType* rsTransform1ToWGS84, RSTransformType* rsTransform2ToWGS84,
             std::ofstream& file, OGRMultiLineString* mls = nullptr)
  {
    ...
    try
    {
      ...
      for (LandmarkListType::Iterator it = landmarks->Begin(); it != landmarks->End(); ++it)
      {
  ...
  void DoExecute() override
  {
    ...
    else if (GetParameterString("mode") == "geobins")
    {
      ...
      for (unsigned int i = 0; i < nb_bins_x; ++i)
      {
        for (unsigned int j = 0; j < nb_bins_y; ++j)
        {
          ...
          otbAppLogINFO("(" << i + 1 << "/" << nb_bins_x << ", " << j + 1 << "/" << nb_bins_y << ") Considering region1: " << region1.GetIndex() << ", "
                            << region1.GetSize());
    ...
    if (IsParameterEnabled("outvector"))
    {
      ...
      std::string projref =
          ...
          "84\",6378137,298.257223563,AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],PRIMEM[\"Greenwich\",0,AUTHORITY[\"EPSG\",\"8901\"]],UNIT["
          "\"degree\",0.01745329251994328,AUTHORITY[\"EPSG\",\"9122\"]],AUTHORITY[\"EPSG\",\"4326\"]]";
