...
private:
  void DoInit() override
  {
    ...
    SetDocLongDescription(
        ...
        "\n"
        ":math:`L = \\begin{cases}  L_{1} & : x_{1}>x_{2} \\quad and \\quad x_{1}>\\sigma \\\\ L_{2} & : x_{2}>x_{1} \\quad and \\quad x_{2}>\\sigma \\\\ 0   "
        "& : otherwise. \\end{cases}` \n"
        "\n"
    ...
    SetDocLimitations(
        "Generation of the morphological profile is not streamable, pay attention to this fact when setting the radius initial size and step of the "
        "structuring element.");
    SetDocAuthors("OTB-Team");
    SetDocSeeAlso(
        "otbMorphologicalOpeningProfileFilter, otbMorphologicalClosingProfileFilter, otbProfileToProfileDerivativeFilter, "
        "otbProfileDerivativeToMultiScaleCharacteristicsFilter, otbMultiScaleConvexOrConcaveClassificationFilter, classes");
  ...
  void performProfileAnalysis(std::string profile, unsigned int profileSize, unsigned short initValue, unsigned short step, float sigma)
  {
    ...
    if (doOpening)
    {
      performOperations<OpeningProfileFilterType, DerivativeFilterType, MultiScaleCharacteristicsFilterType>(
          oprofileFilter, oderivativeFilter, omsCharFilter, opening, derivativeOpening, characOpening, profileSize, step, initValue);
      if (!classify)
        return;
    }
    ...
    if (doClosing)
    {
      performOperations<ClosingProfileFilterType, DerivativeFilterType, MultiScaleCharacteristicsFilterType>(
          cprofileFilter, cderivativeFilter, cmsCharFilter, closing, derivativeClosing, characClosing, profileSize, step, initValue);
      if (!classify)
        return;
    }
  ...
  void performOperations(typename TProfileFilter::Pointer& profileFilter, typename TDerivativeFilter::Pointer& derivativeFilter,
                         typename TCharacteristicsFilter::Pointer& msCharFilter, bool profile, bool derivative, bool characteristics, unsigned int profileSize,
                         unsigned short initValue, unsigned short step)
  {
