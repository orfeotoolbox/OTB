...
//-------------------------------------------------------
  ...
  //Allocate the output buffer memory 
    ...
    if (m_ExternalClassificationSet)
    {
      ...
      LabelledImageRegionIterator  
          trainingImageIt( trainingImage, 
                           outputPtr->GetRequestedRegion() );
      
      while ( !outImageIt.IsAtEnd() )
      {
        LabelledImagePixelType labelvalue = 
            ( LabelledImagePixelType ) trainingImageIt.Get();
    ...
    else //set to random value
    {
      ...
      LabelledImagePixelType randomvalue = 0;
      while ( !outImageIt.IsAtEnd() )
      {
        LabelledImagePixelType randomvalue = 
            ( LabelledImagePixelType ) rand() % m_NumberOfClasses;
...
//-------------------------------------------------------
  ...
  ::ApplyMarkovClassificationFilter()
  {
    ...
    do
    {
      otbDebugMacro(<< "Iteration No." << m_NumberOfIterations);

      this->MinimizeOnce();
      
      otbDebugMacro(<< "m_ErrorCounter/m_TotalNumberOfPixelsInInputImage: " 
...
//           totalNumberOfPixelsInInputImage;  
    ...
    otbDebugMacro(<< "m_NumberOfIterations: " << m_NumberOfIterations);
    otbDebugMacro(<< "m_MaximumNumberOfIterations: " << m_MaximumNumberOfIterations);
    otbDebugMacro(<< "m_ErrorCounter: " << m_ErrorCounter);
    otbDebugMacro(<< "maxNumPixelError: " << maxNumPixelError);
