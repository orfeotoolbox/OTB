...
/** \class LUMImageIO
 *
 * \brief ImageIO object for reading and writing LUM format images
 *
...
private:
  ...
  long int cai_liste_ima_lum(char *repert,char ***tab_ima);
  CAI_OK_KO cai_dest_ima_lum(char *repertoire,char *nom_ima);
  CAI_IMAGE *cai_ouvre_lecture_lum(char *repert,char *nom_image,
					  int  *nb_can, int  *nboct_pix,
					  int  *nb_col, int  *nb_lig);
  CAI_IMAGE *cai_ouvre_modifie_lum(char *repert,char *nom_image,
					  int  *nb_can, int  *nboct_pix,
					  int  *nb_col, int  *nb_lig);
  CAI_OK_KO cai_ferme_lum(CAI_IMAGE *image1);
  CAI_IMAGE *cai_ouvre_creation_lum(char *repert,char *nom_image,
					  int  *nb_can, int  *nboct_pix,
					  int  *nb_col, int  *nb_lig,
					  char* label);
  CAI_OK_KO cai_ecriture_canal_lum(CAI_IMAGE *image1, int *canal, 
         	 int *premiere_ligne, int *nombre_lignes_ecrire,
		 int *nombre_colonnes_ecrire,unsigned char *data_image);
  CAI_OK_KO cai_lecture_canal_lum(CAI_IMAGE *image1, int *canal,
  ...
  int cai_recuptyp_lum( char *type_code, char *SensCode, 
  			int *nbbits, char *cod_pix);
