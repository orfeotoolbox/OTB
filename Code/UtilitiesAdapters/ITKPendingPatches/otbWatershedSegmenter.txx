...
=========================================================================*/
#ifndef __otbWatershedSegmenter_txx
#define __otbWatershedSegmenter_txx
...
void Segmenter<TInputImage>::GenerateData()
{
  ...
  if (itk::NumericTraits<InputPixelType>::is_integer
      && maximum == itk::NumericTraits<InputPixelType>::max())
    {
  ...
  this->LabelMinima(thresholdImage, thresholdImage->GetRequestedRegion(),
                    flatRegions, maximum + itk::NumericTraits<InputPixelType>::One);
...
::CollectBoundaryInformation(flat_region_table_t &flatRegions)
{
  ...
  itk::ImageRegionIterator<ITK_TYPENAME BoundaryType::face_t> faceIt;
  itk::ImageRegionIterator<OutputImageType>              labelIt;
  ...
  for (idx.first = 0; idx.first < ImageDimension; (idx.first)++)
    ...
    for (idx.second = 0; idx.second < 2; (idx.second)++)
      ...
      faceIt = itk::ImageRegionIterator<ITK_TYPENAME BoundaryType::face_t> (face,
                                                                       region);
      labelIt = itk::ImageRegionIterator<OutputImageType> (output, region);
      faceIt = faceIt.Begin();
...
::InitializeBoundary()
{
  ...
  for (idx.first = 0; idx.first < ImageDimension; ++(idx.first))
    ...
    for (idx.second = 0; idx.second < 2; ++(idx.second))
      ...
      faceIt = itk::ImageRegionIterator<ITK_TYPENAME BoundaryType::face_t>
                      ...
                      InputPixelType max)
{
  ...
  bool isSteepest;
  itk::ConstNeighborhoodIterator<InputImageType>          searchIt;
  itk::NeighborhoodIterator<OutputImageType>               labelIt;
  itk::ImageRegionIterator<ITK_TYPENAME BoundaryType::face_t> faceIt;

  BoundaryIndexType idx;
  ImageRegionType region;
  typename itk::ConstNeighborhoodIterator<InputImageType>::RadiusType rad;
  ...
  for (idx.first = 0; idx.first < ImageDimension; ++(idx.first))
    ...
    for (idx.second = 0; idx.second < 2; ++(idx.second))
      ...
      searchIt
        = itk::ConstNeighborhoodIterator<InputImageType> (rad, thresholdImage, region);
      labelIt = itk::NeighborhoodIterator<OutputImageType> (rad, output, region);
      faceIt  = itk::ImageRegionIterator<ITK_TYPENAME BoundaryType::face_t> (face, region);
...
::GenerateConnectivity()
{
  ...
  itk::ConstNeighborhoodIterator<InputImageType> it(rad, this->GetInputImage(),
              ...
              typename Self::flat_region_table_t &flatRegions, InputPixelType Max)
{
  ...
  itk::EquivalencyTable::Pointer equivalentLabels = itk::EquivalencyTable::New();
  ...
  itk::ConstNeighborhoodIterator<InputImageType>   searchIt(rad, img, region);
  itk::NeighborhoodIterator<OutputImageType>       labelIt(rad, output, region);
  nSize   = searchIt.Size();
  nCenter = nSize >> 1;
                  ...
                  ImageRegionType region)
{
  ...
  //
  typename itk::ConstNeighborhoodIterator<InputImageType>::RadiusType rad;
  typename itk::NeighborhoodIterator<OutputImageType>::RadiusType zeroRad;
  for (i = 0; i < ImageDimension; ++i)
    {
    rad[i] = 1;
    zeroRad[i] = 0;
  ...
  itk::ImageRegionIterator<OutputImageType> it(output, region);
...
::UpdateSegmentTable(InputImageTypePointer input, ImageRegionType region)
{
  ...
  unsigned int i, nPos;
  typename itk::NeighborhoodIterator<OutputImageType>::RadiusType hoodRadius;
  ...
  itk::ConstNeighborhoodIterator<InputImageType> searchIt(hoodRadius,input,region);
  itk::NeighborhoodIterator<OutputImageType> labelIt(hoodRadius, output, region);
...
::SetInputImageValues(InputImageTypePointer img,
                 ...
                 InputPixelType value)
{
  itk::ImageRegionIterator<InputImageType> it(img, region);
  it = it.Begin();
  while (! it.IsAtEnd() )
    {
    it.Set(value);
...
::SetOutputImageValues(OutputImageTypePointer img,
                 ...
                 unsigned long value)
{
  itk::ImageRegionIterator<OutputImageType> it(img, region);
  it = it.Begin();
  while (! it.IsAtEnd() )
    {
    it.Set(value);
                   ...
                   itk::EquivalencyTable::Pointer eqTable)
{
  ...
  for (itk::EquivalencyTable::ConstIterator it = eqTable->Begin();
       it != eqTable->End(); ++it)
    {
...
{
  itk::ImageRegionIterator<InputImageType> dIt(destination, destination_region);
  itk::ImageRegionIterator<InputImageType> sIt(source, source_region);
  ...
  if (itk::NumericTraits<InputPixelType>::is_integer)
    ...
    while ( ! dIt.IsAtEnd() )
      ...
      else if (tmp == itk::NumericTraits<InputPixelType>::max())
        {
        dIt.Set(tmp - itk::NumericTraits<InputPixelType>::One);
        }
      else
        {
        dIt.Set(tmp);
...
::GenerateOutputRequestedRegion(itk::DataObject *output)
{
  ...
  itk::ImageBase<ImageDimension> *imgData;
  itk::ImageBase<ImageDimension> *op;
  imgData = dynamic_cast<itk::ImageBase<ImageDimension> * >(output);
  typename TInputImage::RegionType c_reg;

  if (imgData)
    {
    std::vector<itk::ProcessObject::DataObjectPointer>::size_type idx;
    for (idx = 0; idx < this->GetOutputs().size(); ++idx)
      {
      if (this->GetOutputs()[idx] && this->GetOutputs()[idx] != output)
        {
        op = dynamic_cast<itk::ImageBase<ImageDimension>
