...
=========================================================================*/
#ifndef __otbWatershedRelabeler_txx
#define __otbWatershedRelabeler_txx
...
{
  return static_cast<itk::DataObject*>(ImageType::New().GetPointer());
}
...
::GenerateData()
{
  ...
  itk::EquivalencyTable::Pointer eqT = itk::EquivalencyTable::New();
  ...
  //
  itk::ImageRegionIterator<ImageType> it_a(input, output->GetRequestedRegion());
  itk::ImageRegionIterator<ImageType> it_b(output, output->GetRequestedRegion());
  it_a = it_a.Begin();
  it_b = it_b.Begin();
  while (! it_a.IsAtEnd() )
    {
  ...
  if (imgData)
    ...
    for (idx = 0; idx < this->GetOutputs().size(); ++idx)
      ...
      if (this->GetOutputs()[idx] && this->GetOutputs()[idx] != output)
        {
        op = dynamic_cast<itk::ImageBase<ImageDimension> *>(this->GetOutputs()[idx].GetPointer());
