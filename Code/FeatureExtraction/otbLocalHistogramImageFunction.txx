...
{
  OutputType result(this->GetNumberOfHistogramBins());
  ...
  if(!region.Crop(currentImage->GetRequestedRegion()) )
    ...
    extractFilter->Update();
      ...
      for(unsigned int i = 0 ; i < this->GetNumberOfHistogramBins() ; ++i)
        {
        result[i] = histogramGenerator->GetOutput()->GetFrequency(i);
        }
    }
