...
::BuildProblem()
{
  ...
  // Ce sera la methode a surcharger pour pouvoir apprendre a partir
  // d'un pointset ou d'un fichier.
  ...
  //std::cout <<  probl << " examples " << std::endl;
  long int elements = inputImage->GetNumberOfComponentsPerPixel()*probl;
  //std::cout << "Input image has " << inputImage->GetNumberOfComponentsPerPixel() << " components per pixel " << std::endl;
  ...
  while(!trIt.IsAtEnd())
    ...
    if(trIt.Get()!=0)
      {
      double label = static_cast<double>(trIt.Get());
