...
int otbSparseUnmixingImageFilterNewTest ( int argc, char * argv[] )
{
  ...
  typedef otb::SparseUnmixingImageFilter< ImageType, ImageType > FilterType;
...
int otbSparseUnmixingImageFilterTest ( int argc, char * argv[] )
{
  ...
  parser->AddOption( "--Threshold", "Lower threshold for accounting the wavelet coeffs (def. 10)", "-th", 1, false );
  ...
  {
    std::cerr << argv[0] << " performs Wvlt unmixing on 2 images\n";
