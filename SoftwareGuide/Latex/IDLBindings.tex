...
To be able to use OTB classes from within IDL or ENVI, the main requirement is to translate variables from one space to the other one. Few steps are necessary to create the interface for each OTB class you want to use. Several files need to be created.
 ...
 \begin{minipage}[b]{.46\linewidth}
        \centering\includegraphics[width=8cm]{intro.eps}
 \end{minipage}
...
\item \code {IDL\_LIBRARY\_DIRS} that targets an IDL tree directory taht contains the IDL binaries (for example \code {usr/local/rsi/idl\_vXX/bin/bin.linux.x86}, for Linux 64bits target  \code {bin.linux.x86_64}).
\end{itemize}
...
The complete example can be found in the tree example : \code{Code/Sources/Example1/idl_otbcannyfilter.h} and \code{Code/Sources/Example1/idl_otbcannyfilter.cxx}
\subsection{C++ files}
...
Here file_dllExample1 targets Example1 library.
