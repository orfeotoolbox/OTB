...
documentation contains a \code{Sources} folder where we put the C++ files while IDL files are stored in \code{IDLSources}. This documentation will not detail every lines of these files and it is necessary to start with the source provided with this document.
...
The \code{idlotbdef.h} file has to be included as it contains usefull macros.
Finally, we declare the main using the macro \code{ORFEOANDIDL\_API} that will allow the IDL transcription.
Note that \code{void} indicates the fuction does not return anything. The created image is written inside the main function.
...
First we instanciate \code{char*} that will contain input value. Notice that \code{char*} are used because inputs are file names.
\begin{verbatim}
...
Remind that the IDL routines will take only two arguments (input and ouput file name), this explain the "2" in the following extracted code and let's start with the IDL routine definition:
\begin{verbatim}
