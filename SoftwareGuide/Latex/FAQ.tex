...
\section{Special issues about compiling from source OTB}
\label{sec:FAQInstall}

All information about OTB compilation could be found into the related section. We present only here the special issues which can be encountered by some developers.
...
\subsubsection{MacOSX 10.9}
Some developers encounters some issues about the compilation of OTB 4.0 on MacOSX 10.9.
To fix these issue, you should compile with internal ITK to avoid conflict with libKML. To do this, just switch OTB\_USE\_EXTERNAL\_ITK to OFF.
...
\begin{itemize}
\item the version of the library,
\item building them in release mode as much as possible (or RealWitDebInfo). 
\end{itemize}

Currently it is not possible to build OTB in Debug when using the dependencies provided by OSGeo4W. If you want to build OTB in Debug for Windows, you will need to build and install manually each dependency needed by OTB (hard work...). You should use the same compiler for all the dependencies, as much as possible.

Therefore, we highly recommend you to use OSGeo4W shell environment to build OTB. You can use the 32 or 64 bit installer, we provide all the OTB necessary dependencies in the two cases. Please follow carefully the procedure provided in the Software Guide.
...
into the main repository.  You can find more information about the
procedure at
...
\begin{itemize}
  \item{Version 1 (2006):}
    ...
    \item{Version 4.X (2014 and later):}
      \begin{itemize}
	\item support of ITK 4.X,
        \item \ldots
      \end{itemize}

\end{itemize}

You can find more information into RELEASE\_NOTES.txt .
