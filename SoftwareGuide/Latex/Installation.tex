...
This guide will use \texttt{\~/OTB} but you are free to choose something else.
In this directory, there will be three locations:
\begin{itemize}
\item \texttt{\~/OTB/otb} for the source file obtained from the git repository
\item \texttt{\~/OTB/build} for the intermediate build objects, CMake specific files, libraries and binaries.
\item \texttt{\~/OTB/install}, the installation directory for OTB once it is built.
...
\hline
\texttt{BUILD\_DIRECTORY}               & Directory where build files and binaries are put, e.g. \texttt{\~/OTB/build} \\
...
\texttt{\~/OTB/build/Downloads} in our example.
...
You are now ready to compile OTB! Simply use the make command (other targets can be generated with CMake's \texttt{-G}
option):
\begin{verbatim}
$ cd ~/OTB/build
$ make
...
cmake variables
itk and OTB\_USE dependency mechanism
procedure

minimum command with all defaults
cmake -DINSTALL ../qksjfm
...
The CMake variable \code{OTB\_BUILD\_DEFAULT\_MODULES}  If you want to reduce the set of enabled modules, in order to produce a smaller OTB or to avoid some third parties, you have two methods :
\begin{itemize}
