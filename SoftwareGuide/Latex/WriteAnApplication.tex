...
where \code{key} refers to parameter name, defined using \code{AddParameter()} method in \code{DoInit()} method.
...
\doxygen{otb}{Wrapper::Application}. Parameters are set mandatory (or not) in \code{DoInit()} using \code{MandatoryOn(key)} method (\code{MandatoryOff(key)}).
...
In order to compile your application you must call the macro \code{OTB\_CREATE\_APPLICATION} in the \emph{CMakelist.txt} file. This macro generates the lib \emph{otbapp\_XXX.so}, in (OTB\_BINARY\_DIR), which \emph{XXX} refers to the class name.
