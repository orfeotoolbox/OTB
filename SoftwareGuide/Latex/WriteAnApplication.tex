...
a call to the \code{OTB_APPLICATION_EXPORT} macro.
...
is called each time a parameter field is modified. It can be used to maintain consistency 
...
Each created parameter has a unique key and several boolean flags to represent their state. These flags
...
\code{SetParameterRole(key,Role_Output)}.
...
The macro \code{OTB_TEST_APPLICATION} makes it easy to define a new test.
