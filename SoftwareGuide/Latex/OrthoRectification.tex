...
As one may note from the model equations, the height of the ground
...
There exist 2 main types of sensor models. On one hand we have the
...
forward one and vice-versa). They have the big advantage of having
...
needed in the software and that it has to be updated every time a new
sensor is available.\\
...
the case of Pléiades, RPC models have errors lower than 0.02 pixels
with respect to the physical model. Since these models have a standard
form they are easier to use ans implement. However, they have the
...
sensors either throug a physical or an analytical approach. This is
...
transformation has not a unique solution, so over the cartography
