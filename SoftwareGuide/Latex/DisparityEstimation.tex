...
%% compute an analytical deformation model. On the other hand, when
...
%% can not be used. This will be justified in section
%% \ref{sec_correl}. We will thus try to find similarity measures which can be
...
%% In this section, we give several definitions which allow for the
...
%% \section{Geometric deformation modeling\label{sec-model}}
...
%% for the estimation of an analytical model\footnote{However,
%% computation time is not nowadays a real issue. As an illustration, we
%% can give the example of the PAN+XS fusion procedure developped at CNES
%% for the Spot 5 ground segment where the subpixel registration between
%% the Panchromatic band and the multispectral channels is done on
%% $24000\times 24000$ images in less than one hour on a Sun Sparc Ultra-4
%% work station.}. Both of these choices lead to local registration
...
%% \section{Similarity measures\label{sec-simil}}
...
%% \begin{figure}
%% \begin{center}{\includegraphics[width=0.8\hsize]{zones.pdf}}
