/*-----------------------------------------------------------------------*/
/* Makefile COUCHE ACCES IMAGE pour DEC ALPHA (4.0)                      */
/* Version : $Header: cai_bibsl_DEC.mk,v 1.2 01/01/15 15:24:14 cmf Exp $ */
/*-----------------------------------------------------------------------*/

/* Chemins d'acces */

DIRSRC = $(RACINE_ATLAS)/src
DIRBIB = $(RACINE_ATLAS)/bib/bin_DEC
...
${ALL2} : $(DEPBIN) $(DIRINC)/cai_image.h $(JPEGOBJECTS)
	...
	ld -shared -o$(DIRBIB)/cai_couche_ima.so  $(FIC_VERS_OBJ) $(DEPBIN) $(JPEGOBJECTS) `find $(DIRBIB)/HDF -type f -name '*.o' -print`
	@$(RM) -rf $(DIRBIB)/HDF
#endif
#ifdef _NOHDF16
	@echo "--> Generation de la bibliotheque partageable"
	ld -shared -o $(DIRBIB)/cai_couche_ima.so $(FIC_VERS_OBJ) $(DEPBIN) $(JPEGOBJECTS)
#endif
...
$(LIBTIFF):
	( cd $(TIFFDIR) ; make -f Makefile.dec clean ; make -f Makefile.dec CC='$(CC)';cp libtiff.a $(DIRBIB)/.)

$(JPEGOBJECTS):
	( cd  $(JPEGDIR) ;make -f makefile.cai clean; make -f makefile.cai CC='$(CC)' SLFLAG='$(SLFLAGS)')
