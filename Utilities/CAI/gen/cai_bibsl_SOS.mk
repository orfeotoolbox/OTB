/*--------------------------------------------*/
/* Makefile COUCHE ACCES IMAGE pour SUN OS    */
/* Version : $Header: cai_bibsl_SOS.mk,v 1.6 01/01/15 15:24:10 cmf Exp $ */
/*--------------------------------------------*/
...
DIRBIB  = $(RACINE_ATLAS)/bib/bin_SUNOS
...
${ALL2} : $(DEPBIN) $(DIRINC)/cai_image.h $(JPEGOBJECTS)
	...
	@$(CC) -c -fpic $(FLAGS) -o $(FIC_VERS_OBJ) $(FIC_VERS_SRC)
	...
	ld -o $(DIRBIB)/cai_couche_ima.so -assert pure-text $(FIC_VERS_OBJ) $(DEPBIN) $(JPEGOBJECTS) `find $(DIRBIB)/HDF -type f -name '*.o' -print`
	@$(RM) -rf $(DIRBIB)/HDF
#endif
#ifdef _NOHDF16
	@echo "--> Generation de la bibliotheque partageable"
	ld -o $(DIRBIB)/cai_couche_ima.so -assert pure-text $(FIC_VERS_OBJ) $(DEPBIN) $(JPEGOBJECTS)
#endif
...
$(DIRBIB)/cai_DonAux.o : $(DIRSRC)/cai_DonAux.c
	...
	$(CC) -c -fpic $(FLAGS) $(DIRSRC)/cai_DonAux.c -o $@


$(LIBTIFF):
	( cd $(TIFFDIR) ; make -f Makefile.sunos clean ; make  -f Makefile.sunos CC='$(CC)';cp libtiff.a $(DIRBIB))

$(JPEGOBJECTS):
	( cd  $(JPEGDIR) ;make -f makefile.cai clean; make -f makefile.cai CC='$(CC)' SLFLAGS='-fpic')
