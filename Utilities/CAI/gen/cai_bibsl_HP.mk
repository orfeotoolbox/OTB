/*--------------------------------------------*/
/* Makefile COUCHE ACCES IMAGE pour HP	      */
/* Version : $Header: cai_bibsl_HP.mk,v 1.8 01/01/15 15:24:08 cmf Exp $ */
/*--------------------------------------------*/
...
DIRBIB = $(RACINE_ATLAS)/bib/bin_$(MACHINE_ATLAS)
...
RM= rm
CC = cc
CCOPTS	=  -Ae $(CCOPTS64) -D_FILE_OFFSET_BITS=64
AR=ar
SLFLAGS = +Z
LD=ld -b
...
${ALL2} : $(DEPBIN) $(DIRINC)/cai_image.h $(JPEGOBJECTS)
	...
	@$(CC) -c $(CCOPTS) $(FLAGS) -o $(FIC_VERS_OBJ) $(FIC_VERS_SRC)
	...
	$(LD) -o $(DIRBIB)/cai_couche_ima.sl $(FIC_VERS_OBJ) $(DEPBIN) $(JPEGOBJECTS) `find $(DIRBIB)/HDF -type f -name '*.o' -print`
	@$(RM) -rf $(DIRBIB)/HDF
#endif
#ifdef _NOHDF16
	@echo "--> Generation de la bibliotheque partageable"
	$(LD) -o $(DIRBIB)/cai_couche_ima.sl  $(FIC_VERS_OBJ) $(DEPBIN)
#endif
	@$(RM) -f $(FIC_VERS_OBJ) $(FIC_VERS_SRC) $(DEPBIN)
...
$(LIBTIFF):
	( cd $(TIFFDIR) ; make -f Makefile.HPUX clean ; make -f Makefile.HPUX CC='$(CC)' 'CCOPTS=$(CCOPTS)' AR=$(AR) )

$(JPEGOBJECTS):
	( cd  $(JPEGDIR) ;make -f makefile.cai clean; make -f makefile.cai CC='$(CC)' SLFLAG='$(SLFLAGS)' 'CCOPTS=$(CCOPTS)')
