...
//
//*************************************************************************
// $Id: ossimU11ImageData.cpp 15792 2009-10-22 18:03:13Z dburken $
...
void ossimU11ImageData::makeBlank()
{
   ...
   for(ossim_uint32 band =0; band < BANDS; ++band)
   {
      ...
      for (ossim_uint32  i = 0; i < SPB; ++i) p[i] = 0; // zero is our null
   }
...
void ossimU11ImageData::setValue(long x, long y, double color)
{
   if(theDataBuffer.size() > 0 && isWithin(x, y))
   {
      ...
      for(ossim_uint32 band = 0; offset < theDataBuffer.size() && // prevent buffer overrun
                                 band < theNumberOfDataComponents; band++)
      {
