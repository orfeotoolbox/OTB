...
SarSensor::SarSensor(SensorParams* params, PlatformPosition* position, RefPoint* refpoint) : Sensor(params, position, refpoint)
{	
}
...
void SarSensor::ImageToWorld(double distance, JSDDateTime time, double height, double& lon, double& lat)
{
	...
	localisationApriori(*geoEph, lambda, distance, 0.0, sensVisee, RDR_A_WGS84 , RDR_B_WGS84 , height, &cart);

	GeodesicCoordinate geo;
	cart.AsGeodesicCoordinates(RDR_A_WGS84 , RDR_B_WGS84, &geo);
	lon = (geo.get_x())*360.0/RDR_DEUXPI;
	lat = (geo.get_y())*360.0/RDR_DEUXPI;
}
