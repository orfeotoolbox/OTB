                                                ...
                                                ossimGpt&       gpt) const
{
  ...
  {
    // Algorithm from http://wiki.openstreetmap.org/wiki/Slippy_map_tilenames#lon.2Flat_to_tile_numbers
    
    gpt.lon = image_point.samp / (pow(2.0, static_cast<double>(qDepth))*256) * 360.0 - 180;
    double n = M_PI - 2.0 * M_PI * image_point.line / (pow(2.0, static_cast<double>(qDepth))*256);
    gpt.lat = 180.0 / M_PI * atan(0.5 * (exp(n) - exp(-n)));
  }
  else
  {
    gpt.makeNan();
                                          ...
                                          ossimDpt&       img_pt) const
{
  ...
  double x = (ground_point.lon + 180.0) / 360.0 * pow(2.0, static_cast<double>(qDepth));
  double y = (1.0 - log( tan(ground_point.lat * M_PI/180.0) + 1.0 / cos(ground_point.lat * M_PI/180.0)) / M_PI) / 2.0 * pow(2.0, static_cast<double>(qDepth));

  img_pt.samp = floor(x*256);
  img_pt.line = floor(y*256);
