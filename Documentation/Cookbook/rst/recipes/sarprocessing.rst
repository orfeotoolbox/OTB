...
NISAR products preprocessing
----------------------------

NISAR (NASA-ISRO Synthetic Aperture Radar) products are high-resolution radar imagery generated by the joint
NASA-ISRO (Indian Space Research Organisation) mission. These products provide detailed information about Earth's
surface, including topography, vegetation, and surface changes, by using synthetic aperture radar (SAR)
technology. NISAR data is particularly valuable for applications such as disaster monitoring, agricultural
monitoring, and environmental studies, offering consistent and all-weather imaging capabilities.

NISAR products are complex and contain specific metadata and data structures that are not natively supported by
OTB. This limitation arises because OTB's development has not yet incorporated the necessary parsers and handlers,
such developments beeing complexe given the current functionning of the OTB. As a result, users who wish to process
NISAR products with OTB would need to preprocess the data into a format that OTB can read, such as a GeoTIFF.

A `Python script <link_nisar_preparation_>`_ has been developed to facilitate the preprocessing of NISAR data into
GeoTIFF format. This script automates the process of extracting radar backscatter data from NISAR files,
georeferencing the data using metadata, and converting it into a GeoTIFF file that can then be provided as input of
the OTB applciations.

How to use the NISAR preparation script
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Create a specific virtual environment and install the needed dependencies:

.. code-block:: bash

   $ python -m venv otb_nisar_prep
   $ pip install h5py gdal

You are now ready to preprocess your NISAR products:

.. code-block:: bash

   $ python <path_to_script>/nisar_preparation.py <path_to_product>/NISAR_L2_PR_GSLC_002_030_A_019_2800_SHNA_A_20081127T061000_20081127T061014_D00404_N_F_J_001.h5

When the preprocessing is done, you will notice, next to the NISAR product, the TIF files corresponding to each
data layer extracted from the original product.

.. code-block:: bash

   $ ls <path_to_product>
   NISAR_L2_PR_GSLC_002_030_A_019_2800_SHNA_A_20081127T061000_20081127T061014_D00404_N_F_J_001.h5
   NISAR_L2_PR_GSLC_002_030_A_019_2800_SHNA_A_20081127T061000_20081127T061014_D00404_N_F_J_001_frequencyA_HH.tif
   NISAR_L2_PR_GSLC_002_030_A_019_2800_SHNA_A_20081127T061000_20081127T061014_D00404_N_F_J_001_frequencyA_HH.tif.aux.xml

You can now use this TIF file as input of the OTB applications.
